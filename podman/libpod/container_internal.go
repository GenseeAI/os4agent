...
func (c *Container) cleanup(ctx context.Context) error {
	...
	if c.config.ExternalSetup {
		if err := c.RemoveExternalCloneCgroup(); err != nil {
			if lastError != nil {
				logrus.Errorf("Removing container %s clone cgroup: %v", c.ID(), err)
			} else {
				lastError = err
			}
		}

		c.CleanupExternalCloneStorage()
	}
...
func (c *Container) checkExitFile() error {
	...
	if c.config.ExternalSetup && c.state.PID > 0 {
		if _, err := os.Stat(exitFile); err == nil {

		} else if _, perr := os.Stat(fmt.Sprintf("/proc/%d", c.state.PID)); errors.Is(perr, fs.ErrNotExist) {
			c.state.State = define.ContainerStateExited
			c.state.PID = 0
			c.state.ConmonPID = 0
			c.state.FinishedTime = time.Now()
