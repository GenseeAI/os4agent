...
func (r *ConmonOCIRuntime) KillContainer(ctr *Container, signal uint, all bool) error {
	...
	if ctr.config.ExternalSetup {
		if ctr.state.PID > 0 {
			if err := unix.Kill(ctr.state.PID, syscall.Signal(signal)); err != nil && !errors.Is(err, unix.ESRCH) {
				return fmt.Errorf("kill external clone pid %d: %w", ctr.state.PID, err)
			}
		}
		return nil
	}
...
func (r *ConmonOCIRuntime) StopContainer(ctr *Container, timeout uint, all bool) error {
	...
	if ctr.config.ExternalSetup {
		if timeout > 0 {
			...
			if err := unix.Kill(ctr.state.PID, syscall.Signal(stopSignal)); err != nil && !errors.Is(err, unix.ESRCH) {
				return fmt.Errorf("kill external clone pid %d (sig %d): %w", ctr.state.PID, stopSignal, err)
			}
			if err := waitContainerStop(ctr, time.Duration(util.ConvertTimeout(int(timeout)))*time.Second); err == nil {
				return nil
			}
			logrus.Warnf("StopSignal %d failed to stop container %s in %d seconds, resorting to SIGKILL",
				stopSignal, ctr.Name(), timeout)
		}
		if err := unix.Kill(ctr.state.PID, syscall.SIGKILL); err != nil && !errors.Is(err, unix.ESRCH) {
			return fmt.Errorf("sigkill external clone pid %d: %w", ctr.state.PID, err)
		}
		return waitContainerStop(ctr, killContainerTimeout)
	}
...
func (r *ConmonOCIRuntime) AttachSocketPath(ctr *Container) (string, error) {
	...
	if ctr.config.ExternalBundlePath != "" {
		return filepath.Join(ctr.config.ExternalBundlePath, "attach"), nil
	}
...
func waitContainerStop(ctr *Container, timeout time.Duration) error {
	...
	if ctr.config.ExternalSetup {
		return waitPidStopOrZombie(ctr.state.PID, timeout)
	}
	return waitPidStop(ctr.state.PID, timeout)
}

func waitPidStopOrZombie(pid int, timeout time.Duration) error {
	...
	for {
		...
		default:
			...
			if data, err := os.ReadFile(fmt.Sprintf("/proc/%d/stat", pid)); err == nil {
				if rp := bytes.LastIndexByte(data, ')'); rp >= 0 && rp+2 < len(data) && data[rp+2] == 'Z' {
					return nil
				}
			}
