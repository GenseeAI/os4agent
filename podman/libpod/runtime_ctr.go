...
func (r *Runtime) RegisterExternalContainer(ctx context.Context, rSpec *spec.Spec, config *ContainerConfig, pid int) (_ *Container, retErr error) {
	if !r.valid {
		return nil, define.ErrRuntimeStopped
	}
	if config == nil {
		return nil, fmt.Errorf("config is required for external container registration: %w", define.ErrInvalidArg)
	}
	if config.ID == "" {
		return nil, fmt.Errorf("config.ID is required for external container registration: %w", define.ErrInvalidArg)
	}

	preservedLogPath := config.LogPath
	preservedLogDriver := config.LogDriver
	preservedOCIRuntime := config.OCIRuntime
	preservedConmonPidFile := config.ConmonPidFile

	ctr, err := r.initContainerVariables(rSpec, config)
	if err != nil {
		return nil, fmt.Errorf("initializing external container variables: %w", err)
	}

	ctr.restoreFromCheckpoint = false
	ctr.config.ExternalSetup = true
	if preservedLogPath != "" {
		ctr.config.LogPath = preservedLogPath
	}
	if preservedLogDriver != "" {
		ctr.config.LogDriver = preservedLogDriver
	}
	if preservedOCIRuntime != "" {
		ctr.config.OCIRuntime = preservedOCIRuntime
	}
	if preservedConmonPidFile != "" {
		ctr.config.ConmonPidFile = preservedConmonPidFile
	}

	if err := ctr.validate(); err != nil {
		return nil, fmt.Errorf("validating external container: %w", err)
	}

	lock, err := r.lockManager.AllocateLock()
	if err != nil {
		return nil, fmt.Errorf("allocating lock for external container: %w", err)
	}
	ctr.lock = lock
	...
	defer func() {
		if retErr != nil {
			if err := ctr.lock.Free(); err != nil {
				logrus.Errorf("Freeing lock for failed external container: %v", err)
			}
		}
	}()
	...
	if ctr.config.ConmonPidFile == "" {
		ctr.config.ConmonPidFile = filepath.Join(rd, "conmon.pid")
	}
	if ctr.config.PidFile == "" {
		ctr.config.PidFile = filepath.Join(rd, "pidfile")
	}
	if ctr.config.SecretsPath == "" {
		ctr.config.SecretsPath = filepath.Join(ctr.config.StaticDir, "secrets")
		if err := os.MkdirAll(ctr.config.SecretsPath, 0o755); err != nil {
	...
	ctr.state.Mountpoint = ctr.config.Rootfs
	ctr.state.Mounted = true

	if err := r.state.AddContainer(ctr); err != nil {
		return nil, fmt.Errorf("adding external container to state: %w", err)
	}
