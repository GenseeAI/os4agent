...
func (r *Runtime) SetupExternalCloneNetwork(srcCtr, cloneCtr *Container, clonePID int) error {
	if r.network == nil {
		return fmt.Errorf("no network backend configured")
	}
	netNames, err := srcCtr.Networks()
	if err != nil {
		return fmt.Errorf("get source networks: %w", err)
	}
	if len(netNames) == 0 {
		logrus.Infof("tfork: clone %s — source has no networks, leaving clone with empty netns", cloneCtr.ID())
		return nil
	}
	perNet := make(map[string]types.PerNetworkOptions, len(netNames))
	for i, name := range netNames {
		perNet[name] = types.PerNetworkOptions{
			InterfaceName: fmt.Sprintf("eth%d", i),
		}
	}

	procPath := fmt.Sprintf("/proc/%d/ns/net", clonePID)
	cloneNetnsPath, err := netns.NewNSFrom(procPath)
	if err != nil {
		return fmt.Errorf("bind-mount clone netns from %s: %w", procPath, err)
	}
	opts := types.NetworkOptions{
		ContainerID:       cloneCtr.ID(),
		ContainerName:     cloneCtr.Name(),
		Networks:          perNet,
		ContainerHostname: cloneCtr.Hostname(),
		PortMappings:      cloneCtr.config.PortMappings,
	}
	status, err := r.network.Setup(cloneNetnsPath, types.SetupOptions{NetworkOptions: opts})
	if err != nil {
		_ = netns.UnmountNS(cloneNetnsPath)
		return fmt.Errorf("netavark Setup(%s): %w", cloneNetnsPath, err)
	}

	cloneCtr.lock.Lock()
	defer cloneCtr.lock.Unlock()
	cloneCtr.state.NetNS = cloneNetnsPath
	cloneCtr.state.NetworkStatus = status
	if err := cloneCtr.save(); err != nil {
		return fmt.Errorf("save clone network state: %w", err)
	}
	return nil
}

func (r *Runtime) TeardownExternalCloneNetwork(cloneCtr *Container) error {
	if r.network == nil {
		return nil
	}
	netnsPath := cloneCtr.state.NetNS
	if netnsPath == "" {
		return nil
	}
	status := cloneCtr.state.NetworkStatus
	var teardownErr error
	if len(status) > 0 {
		perNet := make(map[string]types.PerNetworkOptions, len(status))
		for netName, sb := range status {
			ifName := ""
			for k := range sb.Interfaces {
				ifName = k
				break
			}
			perNet[netName] = types.PerNetworkOptions{InterfaceName: ifName}
		}
		opts := types.NetworkOptions{
			ContainerID:       cloneCtr.ID(),
			ContainerName:     cloneCtr.Name(),
		...
		if err := r.network.Teardown(netnsPath, types.TeardownOptions{NetworkOptions: opts}); err != nil {
			teardownErr = fmt.Errorf("netavark Teardown(%s): %w", netnsPath, err)
		}
	}
	if err := netns.UnmountNS(netnsPath); err != nil {
		if teardownErr != nil {
			logrus.Errorf("tfork: clone %s netavark Teardown error: %v", cloneCtr.ID(), teardownErr)
		}
		return fmt.Errorf("unmount clone netns %s: %w", netnsPath, err)
	}
	cloneCtr.state.NetNS = ""
	cloneCtr.state.NetworkStatus = nil
	return teardownErr
}
