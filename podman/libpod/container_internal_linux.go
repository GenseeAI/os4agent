...
func (c *Container) cleanupNetwork() error {
	...
	if c.config.ExternalSetup {
		neterr := c.runtime.TeardownExternalCloneNetwork(c)
		if err := c.save(); err != nil {
			if neterr != nil {
				logrus.Errorf("tfork: clone %s teardown network error: %v", c.ID(), neterr)
			}
			return err
		}
		return neterr
...
func (c *Container) MoveExternalCloneToOwnCgroup(clonePID int) error {
	unified, err := cgroups.IsCgroup2UnifiedMode()
	if err != nil {
		return fmt.Errorf("detect cgroup mode: %w", err)
	}
	if !unified {
		logrus.Warnf("tfork: skipping F8 cgroup migration on non-unified host (clone %s stays in source's cgroup)", c.ID())
		return nil
	}
	cgPath := filepath.Join(c.config.CgroupParent, fmt.Sprintf("libpod-%s", c.ID()))
	cgFS := filepath.Join("/sys/fs/cgroup", cgPath)
	if err := os.MkdirAll(cgFS, 0o755); err != nil {
		return fmt.Errorf("mkdir clone cgroup %s: %w", cgFS, err)
	}
	cgProcs := filepath.Join(cgFS, "cgroup.procs")
	moved := 0
	skipped := 0
	...
	for len(stack) > 0 {
		...
		childData, _ := os.ReadFile(fmt.Sprintf("/proc/%d/task/%d/children", pid, pid))
		for _, c := range strings.Fields(string(childData)) {
			var cpid int
			if _, scanErr := fmt.Sscanf(c, "%d", &cpid); scanErr == nil && cpid > 0 {
				stack = append(stack, cpid)
			}
		}
		if writeErr := os.WriteFile(cgProcs, []byte(fmt.Sprintf("%d", pid)), 0); writeErr != nil {
			if pid == clonePID {
				return fmt.Errorf("move clone init PID %d to %s: %w", pid, cgFS, writeErr)
			}
			skipped++
			continue
		}
	...
	logrus.Infof("tfork: clone %s cgroup migration -> %s (moved %d, skipped %d)", c.ID(), cgPath, moved, skipped)
	return nil
}

func (c *Container) RemoveExternalCloneCgroup() error {
	unified, err := cgroups.IsCgroup2UnifiedMode()
	if err != nil {
		return fmt.Errorf("detect cgroup mode: %w", err)
	}
	...
	cgPath := filepath.Join(c.config.CgroupParent, fmt.Sprintf("libpod-%s", c.ID()))
	cgFS := filepath.Join("/sys/fs/cgroup", cgPath)
	if _, err := os.Stat(cgFS); err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("stat clone cgroup %s: %w", cgFS, err)
	}
	if err := os.Remove(cgFS); err != nil {
		logrus.Warnf("tfork: clone %s cgroup rmdir %s skipped: %v", c.ID(), cgFS, err)
		return nil
	}
	logrus.Infof("tfork: clone %s cgroup removed: %s", c.ID(), cgPath)
	return nil
}

func (c *Container) CleanupExternalCloneStorage() {
	...
	if hpid := c.config.TforkDumpdHolderPid; hpid > 0 {
		...
		switch {
		case errors.Is(stErr, unix.ESRCH), os.IsNotExist(stErr):
			logrus.Debugf("tfork: clone %s dumpd-holder pid=%d already gone (no /proc entry)",
				c.ID(), hpid)
		case stErr != nil:
			logrus.Warnf("tfork: clone %s read /proc/%d/stat: %v — skipping holder kill to avoid wrong-target SIGKILL",
				c.ID(), hpid, stErr)
		case expectedStart == 0:

			logrus.Warnf("tfork: clone %s dumpd-holder pid=%d has no recorded start time (legacy state); skipping kill",
				c.ID(), hpid)
		case curStart != expectedStart:
			logrus.Warnf("tfork: clone %s dumpd-holder pid=%d start_time mismatch (have %d, expected %d) — pid was recycled, skipping kill",
				c.ID(), hpid, curStart, expectedStart)
		default:
			err := unix.Kill(-hpid, unix.SIGKILL)
			switch {
			case err == nil:
				logrus.Infof("tfork: clone %s killed dumpd-holder pgid=%d (start_time=%d)",
					c.ID(), hpid, expectedStart)
			case errors.Is(err, unix.ESRCH):
				logrus.Debugf("tfork: clone %s dumpd-holder pgid=%d already gone",
					c.ID(), hpid)
			default:
				logrus.Warnf("tfork: clone %s SIGKILL dumpd-holder pgid=%d: %v",
					c.ID(), hpid, err)
			}
		}
	}
	...
	if os.Getenv("PODMAN_TFORK_NO_REAP") == "1" {
		logrus.Debugf("tfork: clone %s rm: PODMAN_TFORK_NO_REAP=1, preserving bundle %s",
			c.ID(), rootfs)
		return
	}
	...
	suffix := ""
	if strings.HasPrefix(base, "rootfs-") {
		suffix = strings.TrimPrefix(base, "rootfs")
	} else if base != "rootfs" {
		logrus.Debugf("tfork: clone %s rootfs basename %q is neither `rootfs` nor `rootfs-<i>`; skipping storage reap", c.ID(), base)
		return
	}

	if err := unix.Unmount(rootfs, unix.MNT_DETACH); err != nil && !errors.Is(err, fs.ErrNotExist) && err != unix.EINVAL {
		logrus.Debugf("tfork: clone %s umount %s: %v", c.ID(), rootfs, err)
	}

	deletePath(c.ID(), filepath.Join(bundle, "upper"+suffix))
	deletePath(c.ID(), filepath.Join(bundle, "snap"+suffix))
	if err := os.RemoveAll(filepath.Join(bundle, "work"+suffix)); err != nil && !errors.Is(err, fs.ErrNotExist) {
		logrus.Debugf("tfork: clone %s rm work%s: %v", c.ID(), suffix, err)
	}
	deletePath(c.ID(), rootfs)

	lockPath := filepath.Join(bundle, ".cleanup.lock")
	lockF, lerr := os.OpenFile(lockPath, os.O_CREATE|os.O_RDWR, 0o600)
	if lerr != nil {
		logrus.Debugf("tfork: clone %s open cleanup lock %s: %v (skipping shared reap)", c.ID(), lockPath, lerr)
		return
	}
	defer lockF.Close()
	if err := unix.Flock(int(lockF.Fd()), unix.LOCK_EX); err != nil {
		logrus.Debugf("tfork: clone %s flock %s: %v (skipping shared reap)", c.ID(), lockPath, err)
		return
	}
	...
	entries, rerr := os.ReadDir(bundle)
	if rerr != nil {
		logrus.Debugf("tfork: clone %s readdir %s under lock: %v", c.ID(), bundle, rerr)
		return
	}
	anyRootfs := false
	for _, e := range entries {
		if strings.HasPrefix(e.Name(), "rootfs") {
			anyRootfs = true
			break
		}
	}
	...
	snapRO := filepath.Join(bundle, "snap-ro")
	if fi, err := os.Lstat(snapRO); err == nil {
		if fi.Mode()&os.ModeSymlink != 0 {
			_ = os.Remove(snapRO)
		} else {
			deletePath(c.ID(), snapRO)
		}
	}
	deletePath(c.ID(), filepath.Join(bundle, "parent-upper-frozen"))

	if rerr := os.RemoveAll(bundle); rerr == nil {
		logrus.Infof("tfork: clone %s reaped batch bundle %s (last copy)", c.ID(), bundle)
	} else {
		logrus.Debugf("tfork: clone %s rm bundle %s: %v", c.ID(), bundle, rerr)
	}
}

func deletePath(ctrID, path string) {
	if _, err := os.Stat(path); err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			logrus.Debugf("tfork: clone %s stat %s: %v", ctrID, path, err)
		}
		return
	}

	_ = exec.Command("btrfs", "property", "set", "-ts", path, "ro", "false").Run()
	if out, err := exec.Command("btrfs", "subvolume", "delete", path).CombinedOutput(); err != nil {
		if rerr := os.RemoveAll(path); rerr != nil {
			logrus.Debugf("tfork: clone %s delete %s: btrfs %s; rm %v",
				ctrID, path, strings.TrimSpace(string(out)), rerr)
		}
	}
}
