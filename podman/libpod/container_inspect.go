...
func (c *Container) inspectLocked(size bool) (*define.InspectContainerData, error) {
	...
	if c.config.ExternalSetup {
		driverData := &define.DriverData{
			Name: "external-tfork",
			Data: map[string]string{"Source": c.config.Rootfs},
		}
...
func (c *Container) getContainerInspectData(size bool, driverData *define.DriverData) (*define.InspectContainerData, error) {
	...
	if c.config.TforkImgDir != "" || c.config.TforkSourceID != "" {
		dumpStatus, dsErr := c.TforkDumpStatus()
		if dsErr != nil {
			logrus.Warnf("Reading tfork dump status for %s: %v", c.ID(), dsErr)
		}
		data.Tfork = &define.InspectContainerTfork{
			SourceID:       c.config.TforkSourceID,
			ImageDir:       c.config.TforkImgDir,
			Persistent:     c.config.TforkPersistent,
			DumpStatus:     string(dumpStatus),
			ParentCloneID:  c.config.TforkParentClone,
			DumpdHolderPid: c.config.TforkDumpdHolderPid,
		}
	}
...
func (c *Container) generateInspectContainerHostConfig(ctrSpec *spec.Spec, namedVolumes []*ContainerNamedVolume, mounts []spec.Mount) (*define.InspectContainerHostConfig, error) {
	...
	if c.config.CreateNetNS || len(c.config.PortMappings) > 0 {
