...
func (ic *ContainerEngine) ContainerRestore(ctx context.Context, namesOrIds []string, options entities.RestoreOptions) ([]*entities.RestoreReport, error) {
	...
	for _, c := range ctrs {
		...
		dumpStatus, dsErr := c.TforkDumpStatus()
		if dsErr != nil {
			reports = append(reports, &entities.RestoreReport{
				Err:      fmt.Errorf("read tfork dump status: %w", dsErr),
				Id:       c.ID(),
				RawInput: idToRawInput[c.ID()],
			})
			continue
		}
		if dumpStatus == libpod.TforkDumpStateInflight {
			reports = append(reports, &entities.RestoreReport{
				Err:      fmt.Errorf("tfork persistent dump for %s is still in flight (.dump.inflight present, .dump.done missing); wait for the async memdump to complete and retry", c.ID()),
				Id:       c.ID(),
				RawInput: idToRawInput[c.ID()],
			})
			continue
		...
		ancestors, ancErr := c.TforkChainAncestors()
		if ancErr != nil {
			reports = append(reports, &entities.RestoreReport{
				Err:      fmt.Errorf("walk tfork chain: %w", ancErr),
				Id:       c.ID(),
				RawInput: idToRawInput[c.ID()],
			})
			continue
		...
		chainErr := func() error {
			for _, anc := range ancestors {
				ancStatus, err := anc.TforkDumpStatus()
				if err != nil {
					return fmt.Errorf("ancestor %s dump status: %w", anc.ID(), err)
				}
				if ancStatus != libpod.TforkDumpStateDone {
					return fmt.Errorf("tfork chain ancestor %s for %s is %s (incremental restore needs all ancestors)", anc.ID(), c.ID(), ancStatus)
				}
			}
			return nil
		}()
...
func (ic *ContainerEngine) ContainerClone(ctx context.Context, ctrCloneOpts entities.ContainerCloneOptions) (*entities.ContainerCreateReport, error) {
	...
	if ctrCloneOpts.Live {
		return ic.containerCloneLive(ctx, ctrCloneOpts)
	}
