...
static int prepare_sigactions_from_core(TaskCoreEntry *tc)
{
	...
	pr_info("Restore on-core sigactions for %d(%d)\n", realpid(current), localpid(current));
...
static int restore_one_sigaction(int sig, struct cr_img *img, int imguid)
{
	bool sigaction_is_compat;
	SaEntry *e;
	int ret = 0;
...
static int prepare_sigactions_from_image(void)
{
	...
	img = open_image(CR_FD_SIGACT, O_RSTR, imguid);
	if (!img)
		return -1;
	...
	for (sig = 1; sig <= SIGMAX; sig++) {
		...
		ret = restore_one_sigaction(sig, img, imguid);
		if (ret < 0)
			break;
		if (ret)
			rst++;
