...
static int dump_sets(CgroupEntry *cg)
{
	...
	if (opts.mode == CR_TFORK && criu_cgset == root_cgset)
		cg->n_sets = n_sets;
	else
		cg->n_sets = n_sets - 1;
	...
	list_for_each_entry(set, &cg_sets, l) {
		if (set == criu_cgset &&
		    !(opts.mode == CR_TFORK && set == root_cgset))
			continue;
...
int prepare_cgroup_namespace(struct pstree_item *root_task)
{
	 ...
	 *
	 * Under tfork the root cgset IS dumped (see dump_sets()), and we
	 * need prepare_cgns() to fire so the restorer enters the sibling
	 * /<sib_name> cgroup and unshares CLONE_NEWCGROUP — that way the
	 * clone's /proc/self/cgroup reads as /<orig_path> instead of
	 * /<sib_name>/<orig_path>.
	 */
	if (!rsti(root_task)->cg_set ||
	    (!opts.tfork.active && rsti(root_task)->cg_set == root_cg_set)) {
...
static int restore_cgroup_subtree_control(const CgroupPropEntry *cg_prop_entry_p, int fd)
{
	...
	if (!strcmp(buf, cg_prop_entry_p->value))
		return 0;
	...
	for (i = 0; i < cge->n_sets; i++) {
		...
		for (j = 0; j < set->n_ctls; j++) {
			...
			if (cg->has_cgns_prefix && cg->cgns_prefix) {
				...
				char *prev = cg->path;
				/*
				 * Two semantics, picked by sibling_mode
				 * (set when ctrl->n_dirs == 1 under tfork):
				 *
				 * Sibling mode — replace the matched dir
				 * with newroot. With newroot="/sib":
				 *   "/subcg00" -> "/sib"
				 *
				 * Wrapping mode — prepend newroot to the
				 * full path. With newroot="/sib":
				 *   "/subcg01/thread1" -> "/sib/subcg01/thread1"
				 *   "/subcg01/thread2" -> "/sib/subcg01/thread2"
				 * Sibling mode would collapse both to "/sib";
				 * wrapping preserves the per-leaf distinction.
				 *
				 * Sibling mode also synthesizes cgns_prefix
				 * so prepare_cgns() unshares CLONE_NEWCGROUP
				 * with cgns root at /sib. Wrapping can't —
				 * once the restorer enters /sib (with its
				 * threaded subtree_control), /sib becomes an
				 * implicit thread root and descendant attaches
				 * return EOPNOTSUPP.
				 *
				 * Outside tfork: same as sibling mode (legacy
				 * semantic, e.g. systemd perms rewriting on
				 * dump, commit cb19e69e6).
				 */
				if (opts.tfork.active && !sibling_mode)
					cg->path = xsprintf("%s%s", newroot, cg->path);
				else
				...
				xfree(prev);
				if (opts.tfork.active && sibling_mode) {
					cg->has_cgns_prefix = true;
					cg->cgns_prefix = strlen(newroot);
				}
				if (!dirnew) {
					if (opts.tfork.active && !sibling_mode)
						dirnew = xsprintf("%s/%s", newroot, dir);
					else
...
static int rewrite_cgroup_roots(CgroupEntry *cge)
{
	...
	for (i = 0; i < cge->n_controllers; i++) {
		...
		if (newroot) {
			bool sibling_mode = (opts.tfork.active && ctrl->n_dirs == 1);
			for (j = 0; j < ctrl->n_dirs; j++) {
				CgroupDirEntry *cgde = ctrl->dirs[j];

				pr_info("rewriting %s to %s (%s)\n", cgde->dir_name, newroot,
					sibling_mode ? "sibling" : "wrap");
