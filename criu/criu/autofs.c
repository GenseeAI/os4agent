...
static int autofs_create_entry(struct mount_info *pm, AutofsEntry *entry)
{
	...
	if (entry->fd != AUTOFS_CATATONIC_FD) {
		...
		master = pstree_item_by_real(entry->pgrp);
		if (!master) {
			pr_err("failed to find pstree item with pid %d\n", entry->pgrp);
			pr_err("Non-catatonic mount without master?\n");
			return -1;
		}
		entry->pgrp = localpid(master);
		entry->pgrp_pid_ns_id = master->pid->leaf_ns_id;
	}
	return 0;
}
...
static int autofs_dup_pipe(struct pstree_item *task, struct fdinfo_list_entry *ple, int new_fd)
{
	...
	if (dup_fle(task, ple, new_fd, flags) < 0) {
		pr_err("Failed to add fd %d to process %d(%d)\n", new_fd, realpid(task), localpid(task));
		return -1;
	}

	pr_info("autofs: added pipe fd %d, flags %#x to %d(%d)\n", new_fd, flags, realpid(task), localpid(task));
	return new_fd;
}
...
static struct fdinfo_list_entry *autofs_pipe_le(struct pstree_item *master, AutofsEntry *entry)
{
	...
	if (!ple) {
		pr_err("Failed to find pipe fd %d in process %d(%d)\n", pipe_fd, realpid(master), localpid(master));
		return NULL;
	}
	if (ple->fe->type != FD_TYPES__PIPE) {
		pr_err("Fd %d in process %d(%d) is not a pipe: %d\n", pipe_fd, realpid(master), localpid(master), ple->fe->type);
		return NULL;
	}
	return ple;
}
...
static int autofs_create_pipe(struct pstree_item *task, autofs_info_t *i, struct fdinfo_list_entry *ple)
{
	...
	pr_info("autofs: adding pipe fd %d, flags %#x to %d(%d) (with post_open)\n", fe->fd, fe->flags, realpid(task), localpid(task));
	return collect_fd(task, fe, rsti(task), false);
}
...
static int autofs_add_mount_info(struct pprep_head *ph)
{
	...
	if (entry->fd == AUTOFS_CATATONIC_FD)
		...
		master = pstree_item_by_pid_ns(entry->pgrp_pid_ns_id, entry->pgrp);
	BUG_ON(!master);
	...
	if (entry->has_read_fd) {
		...
		if (entry->fd < 0) {
			pr_err("Failed to find free fd in process %d(%d)\n", realpid(master), localpid(master));
			return -1;
		}
	}
