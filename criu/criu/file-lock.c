...
int dump_file_locks(void)
{
	...
	list_for_each_entry(fl, &file_lock_list, list) {
		...
		fle.fd = fl->owners_fd;
		fle.pid_ns_id = fl->real_owner_ns_id;

		if (fle.pid_ns_id == 0) {
			item = pstree_item_by_real(fl->fl_holder);
			if (item) {
				fle.pid = item->pid->local;
				fle.pid_ns_id = item->pid->leaf_ns_id;
			}
		}
...
int note_file_lock(struct pid *pid, int fd, int lfd, struct fd_parms *p)
{
	...
	list_for_each_entry(fl, &file_lock_list, list) {
		...
		fl->real_owner = pid->local;
		fl->real_owner_ns_id = pid->leaf_ns_id;
		fl->owners_fd = fd;

		pr_info("Found lock entry %d.%d %d vs %d\n", pid->real, pid->local, fd, fl->fl_owner);
	}
...
static int restore_file_locks(struct pid *pid)
{
	int ret = 0;
	struct file_lock_rst *lr;

	list_for_each_entry(lr, &file_lock_list, l) {
		if (lr->fle->pid == pid->local && lr->fle->pid_ns_id == pid->leaf_ns_id) {
...
int prepare_file_locks(struct pid *pid)
{
	if (!opts.handle_file_locks)
		return 0;
