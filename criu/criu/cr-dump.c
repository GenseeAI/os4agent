...
static bool needs_cgroup_ns_parasite(const struct pstree_item *item)
{
	if (!item->ids->has_cgroup_ns_id)
		return false;
	if (!item->parent)
		return true;
	return item->parent->ids &&
			      ...
			      const struct cr_imgset *cr_imgset, const struct parasite_dump_misc *misc)
{
	...
	ret = arch_dump_mm_pkey_state(pid, core);
	if (ret)
		goto err;
	...
	if (needs_cgroup_ns_parasite(item)) {
		info = &cgroup_args;
...
static int fixup_thread_rseq(const struct pstree_item *item, int i)
{
	...
	pr_debug(
		...
		item->threads[i].real, rseq_cs->start_ip, rseq_cs->abort_ip, rseq_cs->post_commit_offset, rseq_cs->flags,
...
static int dump_task_thread(struct parasite_ctl *parasite_ctl, const struct pstree_item *item, int id)
{
	...
	if (pstree_insert_pid(tid) < 0) {
		pr_err("Can't insert tid %d into pstree\n", pid);
		goto err;
	}
	...
	img = open_image(CR_FD_CORE, O_DUMP, pid_get_uid(tid));
	if (!img)
		goto err;
...
static int dump_one_zombie(const struct pstree_item *item, const struct proc_pid_stat *pps)
{
	...
	img = open_image(CR_FD_CORE, O_DUMP, uid(item));
	if (!img)
		goto err;
...
static int dump_task_threads(struct parasite_ctl *parasite_ctl, const struct pstree_item *item)
{
	...
	for (i = 0; i < item->nr_threads; i++) {
		...
		if (realpid(item) == item->threads[i].real)
			continue;
...
static int dump_zombies(void)
{
	...
	for_each_pstree_item(item) {
		...
		if (parse_pid_status(realpid(item), &creds.s, NULL) < 0)
			goto err;

		BUG_ON(creds.s.state != 'Z' || creds.s.ns[creds.s.ns_level - 1].ns_pid != realpid(item));
		item->pid->ns_level = creds.s.ns_level;
		memcpy(item->pid->ns, creds.s.ns, creds.s.ns_level * sizeof(struct pid_ns));
		item->pid->local = creds.s.ns[0].ns_pid;

		if (item->parent)
			item->pid->leaf_ns_id = item->parent->pid->leaf_ns_id;
		item->pgid = creds.s.ns[0].pgid;
		item->sid = creds.s.ns[0].sid;
		if (!item->sid) {
			int lvl;
			bool leader_in_ancestor = false;

			for (lvl = 1; lvl < item->pid->ns_level; lvl++) {
				if (item->pid->ns[lvl].sid) {
					leader_in_ancestor = true;
					break;
				}
			}
			if (!leader_in_ancestor) {
				pr_err("A session leader of zombie process %d(%d) is outside of its pid namespace\n",
				       realpid(item), localpid(item));
				goto err;
			}
			pr_info("Zombie %d(%d): session leader lives in ancestor pidns (outer sid=%d)\n",
				realpid(item), localpid(item), item->pid->ns[lvl].sid);
		}

		BUG_ON(item->nr_threads != 1);
		item->threads[0] = *item->pid;
		...
		if (parse_pid_stat(realpid(item), &pps_buf) < 0)
			goto err;
...
static int dump_task_cgroup(struct parasite_ctl *parasite_ctl, const struct pstree_item *item)
{
	...
	for (i = 0; i < item->nr_threads; i++) {
		...
		if (needs_cgroup_ns_parasite(item)) {
			...
			info = &cgroup_args;
			sprintf(cgroup_args.thread_cgrp, "self/task/%d/cgroup", item->threads[i].local);
...
static int dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie)
{
	...
	if (pstree_insert_pid(item->pid) < 0) {
		pr_err("Can't insert pid %d into pstree\n", pid);
		goto err;
	}

	item->sid = item->pid->ns[0].sid;
	item->pgid = item->pid->ns[0].pgid;

	pr_info("sid=%d pgid=%d pid=%d\n", item->sid, item->pgid, realpid(item));
	...
	if (item->sid == 0) {
		...
		pr_info("%d(%d): session leader lives in ancestor pidns (outer sid=%d)\n",
			realpid(item), localpid(item), item->pid->ns[lvl].sid);
	}
	...
	if (item == root_item)
		...
		cflags = get_clone_mask(item->ids, item->parent->ids);

	pr_info("Dump IPC namespace %d via %d\n", item->ids->ipc_ns_id, pid);
	if (cflags & CLONE_NEWIPC) {
		ret = dump_ipc_ns_one_task(pid, item->ids->ipc_ns_id);
		if (ret) {
			pr_err("Dump IPC namespace (pid: %d) failed with %d\n", pid, ret);
			goto err;
		}
	}

	pr_info("Dump TIME namespace time ns %d, and time for children ns %d\n",
		item->ids->time_ns_id, item->ids->time_for_children_ns_id);
	ret = dump_timens_one_task(pid, item->ids->time_ns_id,
			item->ids->time_for_children_ns_id);
	if (ret) {
	...
	if (!opts.tfork.active) {
		...
		if (opts.tfork.memdump_async)
			ret = parasite_fork_dump_pages_seized(item, &vmas, &mdc, parasite_ctl);
		else
			ret = parasite_dump_pages_seized(item, &vmas, &mdc, parasite_ctl);
		if (ret)
			goto err_cure;
	 ...
	 * for lazy pages.
	 */
	if (opts.lazy_pages && !opts.tfork.active)
...
int cr_dump_tasks(pid_t pid)
{
	...
	if (!opts.tfork.active && connect_to_page_server_to_send() < 0)
		goto err;
	...
	if (!opts.tfork.active && rpc_query_external_files())
		goto err;
	...
	if (opts.tfork.active) {
		unsigned long required_ns = CLONE_NEWPID | CLONE_NEWNS | CLONE_NEWNET;
		unsigned long missing_ns = required_ns & ~root_ns_mask;

		if (missing_ns) {
			pr_err("tfork: target missing required namespaces:");
			if (missing_ns & CLONE_NEWPID)
				pr_err("  pid");
			if (missing_ns & CLONE_NEWNS)
				pr_err("  mnt");
			if (missing_ns & CLONE_NEWNET)
				pr_err("  net");
			pr_err("\n");
			goto err;
		}
	 ...
	 * all this.
	 */
	if (!opts.tfork.active && dead_pid_conflict())
		goto err;
	 ...
	 * because page_ids is a real variable and it is used to dump
	 * ipc shared memory, but an ipc namespace is dumped in a child
	 * process.
	 */
	if (!opts.tfork.active && cr_dump_shmem())
		goto err;
