...
static int move_mount_to_tree(struct mount_info *mi)
{
	...
	mi->mp_fd_id = fdstore_add_at(fd, FDSTORE_CANONICAL_ID(FDSTORE_P_MP_FD, mi->mnt_id));
	close(fd);
	if (mi->mp_fd_id < 0) {
	...
	mi->mnt_fd_id = fdstore_add_at(fd, FDSTORE_CANONICAL_ID(FDSTORE_P_MNT_FD, mi->mnt_id));
	close(fd);
	if (mi->mnt_fd_id < 0) {
...
static int pre_create_mount_namespaces(void)
{
	...
	for (nsid = ns_ids; nsid != NULL; nsid = nsid->next) {
		...
		nsid->mnt.nsfd_id = fdstore_add_at(nsfd, FDSTORE_CANONICAL_ID(FDSTORE_P_MNTNS_NSFD, nsid->id));
		close(nsfd);
...
static int assemble_mount_namespaces(void)
{
	...
	for (nsid = ns_ids; nsid != NULL; nsid = nsid->next) {
		...
		if (rootfd < 0)
			goto err;
		nsid->mnt.root_fd_id = fdstore_add_at(rootfd, FDSTORE_CANONICAL_ID(FDSTORE_P_MNTNS_ROOT, nsid->id));
