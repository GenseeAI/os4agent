...
int parse_smaps(pid_t pid, struct vm_area_list *vma_area_list, dump_filemap_t dump_filemap)
{
	...
	while (1) {
		...
		if (!opts.tfork.active &&
		    vma_area && vma_area_is(vma_area, VMA_AREA_VVAR) &&
...
#define PARSE_NS_IDS(token_start, ptr, field, nested_level)         \
({                                                                  \
    int _ret = 0;                                                   \
    int _idx = (nested_level) - 1;                                  \
    const char *_token = (token_start);                             \
                                                                    \
    while (_token != NULL) {                                        \
        if (_idx < 0) {                                             \
            pr_err("Too many IDs in NS line: %s\n", (token_start)); \
            _ret = -1;                                              \
            break;                                                  \
        }                                                           \
                                                                    \
        if (sscanf(_token, "%d", &(ptr)->ns[_idx].field) != 1) {    \
            pr_err("Failed to parse pid: %s\n", _token);            \
            _ret = -1;                                              \
            break;                                                  \
        }                                                           \
        pr_debug("Parsed NS ID [%d]: %d\n", _idx, (ptr)->ns[_idx].field); \
        _idx--;                                                     \
                                                                    \
        _token = strchr(_token + 1, '\t');                          \
    }                                                               \
                                                                    \
    if (_ret == 0 && _idx != -1) {                                  \
        pr_err("Unexpected NS nested level: %s\n", (token_start));  \
        _ret = -1;                                                  \
    }                                                               \
    _ret;                                                           \
})
...
int parse_pid_status(pid_t pid, struct seize_task_status *ss, void *data)
{
	...
	while (done < 17) {
		...
		if (nested_ns_level == 0 &&
			(!strncmp(str, "NSpid:", 6) ||
			 !strncmp(str, "NSpgid:", 7) ||
			 !strncmp(str, "NSsid:", 6))) {
			...
			cr->s.ns_level = nested_ns_level;
			BUG_ON(nested_ns_level > MAX_PID_NS_LEVEL);
			pr_info("Process %d has %d nested PID namespaces\n", pid, cr->s.ns_level);
		}

		if (!strncmp(str, "NSpid:", 6)) {
			if (PARSE_NS_IDS(str + 7, &cr->s, ns_pid, nested_ns_level))
				goto err_parse;
		...
		if (!strncmp(str, "NSpgid:", 7)) {
			if (PARSE_NS_IDS(str + 8, &cr->s, pgid, nested_ns_level))
				goto err_parse;
		...
		if (!strncmp(str, "NSsid:", 6)) {
			if (PARSE_NS_IDS(str + 7, &cr->s, sid, nested_ns_level))
				goto err_parse;
...
int parse_file_locks(void)
{
	...
	while (fgets(buf, BUF_SIZE, fl_locks)) {
		...
		if ((fl->fl_kind == FL_POSIX) && pstree_item_by_real(fl->fl_owner) == NULL) {
			/*
