...
#define BOOL_OPT(OPT_NAME, SAVE_TO)                         \
	...
	static struct option long_opts[] = {
		...
		BOOL_OPT(OPT_KEEP_PID_HIERARCHY, &opts.keep_pid_hierarchy),
		...
		BOOL_OPT(UNIX_SK_INFLIGHT_PARAM, &opts.unix_skip_in_flight),
		...
		{ "tfork-restore", no_argument, 0, 1101 },
		{ "tfork-snap-root", required_argument, 0, 1102 },
		{ "tfork-memdump", no_argument, 0, 1103 },
		{ "tfork-memdump-async", no_argument, 0, 1104 },
		{ "tfork-copies", required_argument, 0, 1105 },
		{ "tfork-snap-roots", required_argument, 0, 1106 },
		{ "tfork-copy", required_argument, 0, 1107 },
		{ "tfork-snap-mount", required_argument, 0, 1108 },
		{ "tfork-snap-mounts", required_argument, 0, 1109 },
		{ "tfork-dumpd-parent", required_argument, 0, 1110 },
		{ "tfork-full-memcopy", no_argument, 0, 1111 },
		{},
	};
...
#undef BOOL_OPT
	...
	while (1) {
		...
		case 1101:
			...
			break;
		case 1102:
			opts.tfork.snap_root = xstrdup(optarg);
		...
		case 1104:
			...
			break;
		case 1105:
			opts.tfork.copies = atoi(optarg);
			if (opts.tfork.copies < 1) {
				pr_err("--tfork-copies must be >= 1 (got %s)\n", optarg);
				return 1;
			}
			break;
		case 1106: {
			...
			if (!dup)
				return 1;
			opts.tfork.snap_roots = xmalloc(cap * sizeof(*opts.tfork.snap_roots));
			...
			while (tok) {
				if (i >= cap) {
					cap *= 2;
					opts.tfork.snap_roots = xrealloc(
						opts.tfork.snap_roots,
						cap * sizeof(*opts.tfork.snap_roots));
					if (!opts.tfork.snap_roots) {
						xfree(dup);
						return 1;
					}
				}
				opts.tfork.snap_roots[i] = xstrdup(tok);
			...
			opts.tfork.snap_roots_n = i;
			xfree(dup);
			break;
		}
		case 1107: {
			char *sep = strstr(optarg, "::");
			char *endp = NULL;
			long idx;
			char *args_dup;
			if (!sep || sep == optarg) {
				pr_err("--tfork-copy: expected <i>::<args>, got `%s`\n", optarg);
				return 1;
			}
			*sep = '\0';
			idx = strtol(optarg, &endp, 10);
			if (endp == optarg || *endp != '\0' || idx < 0) {
				pr_err("--tfork-copy: bad copy index in `%s`\n", optarg);
				*sep = ':';
				return 1;
			}
			*sep = ':';
			args_dup = xstrdup(sep + 2);
			if (!args_dup)
				return 1;
			if (idx >= opts.tfork.per_copy_args_n) {
				int new_cap = idx + 1;
				char **grown = xrealloc(opts.tfork.per_copy_args,
							new_cap * sizeof(*opts.tfork.per_copy_args));
				if (!grown) {
					xfree(args_dup);
					return 1;
				}

				for (int j = opts.tfork.per_copy_args_n; j < new_cap; j++)
					grown[j] = NULL;
				opts.tfork.per_copy_args = grown;
				opts.tfork.per_copy_args_n = new_cap;
			}
			if (opts.tfork.per_copy_args[idx]) {

				size_t old_len = strlen(opts.tfork.per_copy_args[idx]);
				size_t new_len = strlen(args_dup);
				char *concat = xmalloc(old_len + 1 + new_len + 1);
				if (!concat) {
					xfree(args_dup);
					return 1;
				}
				memcpy(concat, opts.tfork.per_copy_args[idx], old_len);
				concat[old_len] = ' ';
				memcpy(concat + old_len + 1, args_dup, new_len);
				concat[old_len + 1 + new_len] = '\0';
				xfree(opts.tfork.per_copy_args[idx]);
				xfree(args_dup);
				opts.tfork.per_copy_args[idx] = concat;
			} else {
				opts.tfork.per_copy_args[idx] = args_dup;
			}
			break;
		}
		case 1108: {
			...
			grown[opts.tfork.snap_mount_n++] = dup;
		...
		case 1109: {
			...
			if (!dup)
				return 1;
			opts.tfork.snap_mounts = xmalloc(cap * sizeof(*opts.tfork.snap_mounts));
			if (!opts.tfork.snap_mounts) {
				xfree(dup);
				return 1;
			}
			tok = strtok_r(dup, ",", &saveptr);
			while (tok) {
				if (i >= cap) {
					cap *= 2;
					opts.tfork.snap_mounts = xrealloc(
						opts.tfork.snap_mounts,
						cap * sizeof(*opts.tfork.snap_mounts));
					if (!opts.tfork.snap_mounts) {
						xfree(dup);
						return 1;
					}
				}
				opts.tfork.snap_mounts[i] = xstrdup(tok);
				if (!opts.tfork.snap_mounts[i]) {
					xfree(dup);
					return 1;
				}
				i++;
		...
		case 1110: {
			...
			if (errno || *end != '\0' || v <= 0 || v > INT_MAX) {
				pr_err("--tfork-dumpd-parent: expected positive PID, got `%s`\n",
				       optarg);
				return 1;
			}
			opts.tfork.dumpd_parent_pid = (pid_t)v;
			break;
		}
		case 1111:
...
int check_options(void)
{
	...
	if (opts.unix_skip_in_flight)
		pr_info("Will skip in-flight unix-socket icon write queues\n");
	if (opts.tcp_close)
	...
	if (opts.mntns_compat_mode && opts.mode != CR_RESTORE && opts.mode != CR_TFORK) {
		pr_err("Option --mntns-compat-mode is only valid on restore\n");
		return 1;
	} else if (!opts.mntns_compat_mode &&
		   (opts.mode == CR_RESTORE || opts.mode == CR_TFORK)) {
		if (check_mount_v2()) {
