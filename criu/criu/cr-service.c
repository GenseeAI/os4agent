...
static int send_criu_tfork_resp(int socket_fd, bool success)
{
	CriuResp msg = CRIU_RESP__INIT;

	msg.type = CRIU_REQ_TYPE__TFORK;
	msg.success = success;
	set_resp_err(&msg);
...
static int setup_opts_from_req(int sk, CriuOpts *req)
{
	...
	if (getenv("CRIU_FORCE_EXT_UNIX_SK"))
	...
	if (req->has_link_remap && req->link_remap)
		opts.link_remap_ok = 1;
	...
	if (req->tfork_snap_root) {
		opts.tfork.snap_root = xstrdup(req->tfork_snap_root);
		if (!opts.tfork.snap_root)
			goto err;
	}

	if (req->n_tfork_snap_roots) {
		size_t i;

		opts.tfork.snap_roots = xmalloc(req->n_tfork_snap_roots *
						sizeof(*opts.tfork.snap_roots));
		if (!opts.tfork.snap_roots)
			goto err;
		for (i = 0; i < req->n_tfork_snap_roots; i++) {
			opts.tfork.snap_roots[i] = xstrdup(req->tfork_snap_roots[i]);
			if (!opts.tfork.snap_roots[i])
				goto err;
		}
		opts.tfork.snap_roots_n = req->n_tfork_snap_roots;
	}

	if (req->n_tfork_snap_mount) {
		size_t i;

		opts.tfork.snap_mount = xmalloc(req->n_tfork_snap_mount *
						sizeof(*opts.tfork.snap_mount));
		if (!opts.tfork.snap_mount)
			goto err;
		for (i = 0; i < req->n_tfork_snap_mount; i++) {
			opts.tfork.snap_mount[i] = xstrdup(req->tfork_snap_mount[i]);
			if (!opts.tfork.snap_mount[i])
				goto err;
		}
		opts.tfork.snap_mount_n = req->n_tfork_snap_mount;
	}

	if (req->has_tfork_memdump && req->tfork_memdump)
		opts.tfork.memdump = true;

	if (req->has_tfork_memdump_async && req->tfork_memdump_async) {
		opts.tfork.memdump = true;
		opts.tfork.memdump_async = true;
	}

	if (req->has_tfork_copies) {
		if (req->tfork_copies < 1) {
			pr_err("tfork_copies must be >= 1 (got %d)\n", req->tfork_copies);
			goto err;
		}
		opts.tfork.copies = req->tfork_copies;
	}

	if (req->has_tfork_dumpd_parent_pid) {
		if (req->tfork_dumpd_parent_pid <= 0) {
			pr_err("tfork_dumpd_parent_pid must be > 0 (got %d)\n",
			       req->tfork_dumpd_parent_pid);
			goto err;
		}
		opts.tfork.dumpd_parent_pid = req->tfork_dumpd_parent_pid;
	}

	if (req->has_tfork_full_memcopy && req->tfork_full_memcopy)
		opts.tfork.full_memcopy = true;

	if (opts.tfork.snap_roots_n != 0 && opts.tfork.copies > 1 &&
	    opts.tfork.snap_roots_n != opts.tfork.copies) {
		pr_err("tfork_snap_roots has %d entries but tfork_copies=%d\n",
		       opts.tfork.snap_roots_n, opts.tfork.copies);
		goto err;
	}

	if (req->n_tfork_copy_args > 0) {
		size_t i;
		int max_idx = -1;

		for (i = 0; i < req->n_tfork_copy_args; i++) {
			CriuTforkCopyArgs *e = req->tfork_copy_args[i];
			if (e->copy_idx < 0) {
				pr_err("tfork_copy_args[%zu]: copy_idx %d < 0\n",
				       i, e->copy_idx);
				goto err;
			}
			if (e->copy_idx > max_idx)
				max_idx = e->copy_idx;
		}
		opts.tfork.per_copy_args_n = max_idx + 1;
		opts.tfork.per_copy_args = xmalloc(opts.tfork.per_copy_args_n *
						   sizeof(*opts.tfork.per_copy_args));
		if (!opts.tfork.per_copy_args)
			goto err;
		for (i = 0; i < (size_t)opts.tfork.per_copy_args_n; i++)
			opts.tfork.per_copy_args[i] = NULL;
		for (i = 0; i < req->n_tfork_copy_args; i++) {
			CriuTforkCopyArgs *e = req->tfork_copy_args[i];
			char *new_args = xstrdup(e->args);
			if (!new_args)
				goto err;
			if (opts.tfork.per_copy_args[e->copy_idx]) {

				char *prev = opts.tfork.per_copy_args[e->copy_idx];
				size_t old_len = strlen(prev);
				size_t new_len = strlen(new_args);
				char *concat = xmalloc(old_len + 1 + new_len + 1);
				if (!concat) {
					xfree(new_args);
					goto err;
				}
				memcpy(concat, prev, old_len);
				concat[old_len] = ' ';
				memcpy(concat + old_len + 1, new_args, new_len);
				concat[old_len + 1 + new_len] = '\0';
				xfree(prev);
				xfree(new_args);
				opts.tfork.per_copy_args[e->copy_idx] = concat;
			} else {
				opts.tfork.per_copy_args[e->copy_idx] = new_args;
			}
		}
	}
...
static int tfork_using_req(int sk, CriuOpts *req)
{
	bool success = false;
	...
	__setproctitle("tfork --rpc -t %d", req->pid);

	if (init_pidfd_store_hash())
		goto pidfd_store_err;

	if (cr_tfork_tasks(req->pid))
		goto exit;
...
exit:
	free_pidfd_store();
pidfd_store_err:
	if (send_criu_tfork_resp(sk, success) == -1) {
		pr_perror("Can't send response");
		success = false;
	}
...
int cr_service_work(int sk)
{
	...
	if (prctl(PR_SET_PDEATHSIG, SIGKILL, 0, 0, 0) < 0)
		pr_pwarn("PR_SET_PDEATHSIG failed");
...
more:
	...
	case CRIU_REQ_TYPE__DUMP:
		...
		break;
	case CRIU_REQ_TYPE__TFORK:
		ret = tfork_using_req(sk, msg->opts);
		break;
