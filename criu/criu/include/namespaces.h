...
#define CLONE_SUBNS (CLONE_NEWNS | CLONE_NEWNET | CLONE_NEWIPC | CLONE_NEWUTS | CLONE_NEWUSER | CLONE_NEWTIME | CLONE_NEWCGROUP | CLONE_NEWPID)
...
extern int userns_addr_init(void);
extern void userns_addr_publish(unsigned int user_ns_id);
extern int userns_addr_open_nsfd(unsigned int user_ns_id);

extern uid_t userns_uid(uid_t uid);
extern gid_t userns_gid(gid_t gid);

extern int dump_user_ns(struct ns_id *ns);
extern void free_userns_maps(void);
extern unsigned int find_host_user_ns_id(void);
...
extern unsigned int proc_read_ns_kid(int pid, struct ns_desc *nd);
extern struct ns_id *proc_read_ns(int pid, struct ns_desc *nd);
