...
{
	return pid_rst_prio(fd_a->real_pid, fd_b->real_pid) || ((fd_a->real_pid == fd_b->real_pid) && (fd_a->fe->fd < fd_b->fe->fd));
}
...
int collect_fd(struct pstree_item *item, FdinfoEntry *e, struct rst_info *rst_info, bool ghost);
...
extern int inherit_fd_for_each(int (*cb)(int fd, const char *key, void *arg), void *arg);
...
extern int set_fds_event(struct pstree_item *item);
