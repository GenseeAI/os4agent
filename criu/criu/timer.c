...
int prepare_itimers_from_fd(int uid, struct task_restore_args *args)
{
	int ret = -1;
	struct cr_img *img;
	ItimerEntry *ie;
	...
	img = open_image(CR_FD_ITIMERS, O_RSTR, uid);
	if (!img)
		return -1;
...
int prepare_posix_timers_from_fd(int uid, struct task_restore_args *ta)
{
	struct cr_img *img;
	int ret = -1;
	...
	img = open_image(CR_FD_POSIX_TIMERS, O_RSTR, uid);
	if (!img)
		return -1;
...
int prepare_posix_timers(int uid, struct task_restore_args *ta, CoreEntry *core)
{
	int i, ret = -1;
	TaskTimersEntry *tte = core->tc->timers;
	struct restore_posix_timer *t;
	...
	if (!tte)
		return prepare_posix_timers_from_fd(uid, ta);
