...
#define SYSVIPC_SHMEM_PID (NULL)
static inline bool is_sysvipc_shmem(const struct shmem_info *si)
{
	return si->pid == SYSVIPC_SHMEM_PID;
}
...
int collect_shmem(struct pid *pid, struct vma_area *vma)
{
	VmaEntry *vi = vma->e;
	...
	if (si) {
		 ...
		 * into /proc
		 */
		if (!pid_rst_prio(pid->real, si->pid->real)) {
			if (pid->real == si->pid->real)
				si->self_count++;
...
static int shmem_wait_and_open(struct shmem_info *si, VmaEntry *vi)
{
	...
	pr_info("Opening shmem [/proc/%d/fd/%d] from pid: %d(%d)\n",
		si->pid->local, si->fd, si->pid->real, si->pid->local);
	ret = open_proc_rw(si->pid->local, "fd/%d", si->fd);
...
static int open_shmem(int real_pid, struct vma_area *vma)
{
	VmaEntry *vi = vma->e;
	struct shmem_info *si;
	...
	si = shmem_find(vi->shmid);
	pr_info("Search for %#016" PRIx64 " shmem 0x%" PRIx64 " %p/%d\n", vi->start, vi->shmid, si, si ? si->pid->real : -1);
	if (!si) {
		pr_err("Can't find my shmem %#016" PRIx64 "\n", vi->start);
		return -1;
	}

	BUG_ON(is_sysvipc_shmem(si));

	if (si->pid->real != real_pid)
...
int add_shmem_area(struct pid *pid, VmaEntry *vma, pmc_t *pmc)
{
	struct shmem_info *si;
	...
	si->pid = vma_entry_is(vma, VMA_AREA_SYSVIPC) ? SYSVIPC_SHMEM_PID : pid;
	si->start = vma->start;
	si->end = vma->end;
	si->shmid = vma->shmid;
	shmem_hash_add(si);
...
static int dump_one_shmem(struct shmem_info *si)
{
	...
	fd = __open_proc(si->pid->real, EPERM, O_RDONLY, "map_files/%lx-%lx", si->start, si->end);
	if (fd >= 0) {
