...
void arch_free_thread_info(CoreEntry *core)
{
	...
	xfree(core->thread_info->mm_pkey_state);
	xfree(core->thread_info);
}

int arch_dump_mm_pkey_state(pid_t pid, CoreEntry *core)
{
	ThreadInfoX86 *ti = core->thread_info;
	MmPkeyStateX86Entry *p;
	uint32_t map;
	int32_t exec_only;
	...
	if (!pkey_state_available())
		return 0;
	if (!ti) {
		pr_err("arch_dump_mm_pkey_state: no thread_info on core for pid %d\n", pid);
		return -1;
	}

	if (pkey_state_get(pid, &map, &exec_only) < 0)
		return -1;

	p = xmalloc(sizeof(*p));
	if (!p)
		return -1;
	mm_pkey_state_x86_entry__init(p);
	p->allocation_map = map;
	p->execute_only_pkey = exec_only;

	ti->mm_pkey_state = p;
	pr_debug("dumped mm_pkey_state pid=%d map=0x%x exec_only=%d\n",
		 pid, map, exec_only);
	return 0;
}

int arch_restore_mm_pkey_state(pid_t pid, CoreEntry *core)
{
	MmPkeyStateX86Entry *p;

	if (!core || !core->thread_info)
		return 0;
	p = core->thread_info->mm_pkey_state;
	if (!p)
		return 0;

	return pkey_state_set(pid, p->allocation_map, p->execute_only_pkey);
}
