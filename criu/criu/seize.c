...
static int collect_children(struct pstree_item *item)
{
	...
	for (i = 0; i < nr_children; i++) {
		...
		c->pid->real = pid;
		c->pid->ns_level = creds.s.ns_level;
		memcpy(c->pid->ns, creds.s.ns, creds.s.ns_level * sizeof(struct pid_ns));
		c->pid->local = creds.s.ns[0].ns_pid;
		c->pgid = c->pid->ns[0].pgid;
		c->sid = c->pid->ns[0].sid;
		c->parent = item;
		c->pid->state = ret;
...
static int collect_threads(struct pstree_item *item)
{
	...
	if (item->nr_threads == 0) {
		...
		item->threads[0].local = item->pid->local;
		item->threads[0].uid = item->pid->uid;
		item->threads[0].ns_level = item->pid->ns_level;
		memcpy(item->threads[0].ns, item->pid->ns, item->pid->ns_level * sizeof(struct pid_ns));
	...
	for (i = 0; i < nr_threads; i++) {
		...
		item->threads[item->nr_threads].ns_level = t_creds.s.ns_level;
		memcpy(item->threads[item->nr_threads].ns, t_creds.s.ns,
			t_creds.s.ns_level * sizeof(struct pid_ns));
		item->threads[item->nr_threads].local = t_creds.s.ns[0].ns_pid;
		item->threads[item->nr_threads].uid = atomic_inc_return(&pid_uid_generator);
...
int collect_pstree(void)
{
	...
	root_item->pid->ns_level = creds.s.ns_level;
	memcpy(root_item->pid->ns, creds.s.ns,
	       creds.s.ns_level * sizeof(struct pid_ns));
	root_item->pid->local = creds.s.ns[0].ns_pid;
	root_item->pgid = root_item->pid->ns[0].pgid;
	root_item->sid = root_item->pid->ns[0].sid;
