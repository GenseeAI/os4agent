			     ...
			     pmc_t *pmc, bool has_parent, bool pre_dump, int parent_predump_mode)
{
	...
	if (vma_area_is(vma, VMA_ANON_SHARED))
		return add_shmem_area(item->pid, vma->e, pmc);
	vaddr = vma->e->start;
again:
					...
					struct parasite_ctl *ctl)
{
	...
	if (!mdc->pre_dump) {
		 ...
		 */
		ret = open_page_xfer(&xfer, CR_FD_PAGEMAP, uid(item));
		if (ret < 0)
			goto out_pp;

		xfer.transfer_lazy = !mdc->lazy;
	} else {
		ret = check_parent_page_xfer(CR_FD_PAGEMAP, uid(item));
		if (ret < 0)
			goto out_pp;
...
static bool helper_dumps_vma(struct vma_area *vma)
{
	if (!vma_area_is_private(vma, kdat.task_size))
		return false;
	if (!(vma->e->prot & PROT_READ))
	...
	if (vma_entry_is(vma->e, VMA_AREA_AIORING))
		return false;
	if (vma->e->has_madv && (vma->e->madv &
	    ((1ull << MADV_DONTFORK) | (1ull << MADV_WIPEONFORK))))
		return false;
	return true;
}

int parasite_fork_dump_pages_seized(struct pstree_item *item,
				    struct vm_area_list *vma_area_list,
				    struct mem_dump_ctl *mdc,
				    struct parasite_ctl *ctl)
{
	struct parasite_fork_dump_pages_args *args;
	void *iovs_map = MAP_FAILED;
	...
	pmc_t pmc = PMC_INIT;
	bool pmc_inited = false;
	int ret;
	int helper_pidfd = -1;
	int image_fd = -1;
	int iovs_fd = -1;
	size_t iovs_bytes = 0;
	size_t aio_total_bytes = 0;
	size_t aio_off = 0;
	size_t mmap_bytes = 0;
	unsigned int nr_aios = 0;
	char ack;
	unsigned long max_iovs = 0;
	unsigned int idx = 0;
	u32 pages_id = 0;
	bool has_parent = false;
	int possible_pid_reuse = 0;
	unsigned long nr_pe_present = 0, nr_pe_parent = 0;

	list_for_each_entry(vma, &vma_area_list->h, list) {
		if (helper_dumps_vma(vma))
			max_iovs += (vma->e->end - vma->e->start) / PAGE_SIZE;
		else if (vma_entry_is(vma->e, VMA_AREA_AIORING)) {
			max_iovs += 1;
			aio_total_bytes += vma->e->end - vma->e->start;
			nr_aios++;
		}
	}

	args = compel_parasite_args(ctl, struct parasite_fork_dump_pages_args);
	args->child_pid = -1;
	args->image_fd = -1;
	args->iovs_fd = -1;
	args->nr_iovs = 0;
	...
	pmi = open_image(CR_FD_PAGEMAP, O_DUMP, uid(item));
	if (!pmi) {
		pr_err("Async memdump: open pagemap-<%d>.img failed\n",
		       uid(item));
		return -1;
	}
	pi = open_pages_image(O_DUMP, pmi, &pages_id);
	if (!pi) {
		pr_err("Async memdump: open_pages_image failed (task %d)\n",
		       uid(item));
		close_image(pmi);
		return -1;
	}

	if (mdc && opts.track_mem && opts.img_parent) {
		ret = check_parent_page_xfer(CR_FD_PAGEMAP, uid(item));
		if (ret < 0) {
			pr_err("Async memdump: check_parent_page_xfer failed "
			       "(task %d)\n", uid(item));
			close_image(pi);
			close_image(pmi);
			return -1;
		...
		if (ret == 1) {
			possible_pid_reuse = detect_pid_reuse(item, mdc->stat,
							      mdc->parent_ie);
			if (possible_pid_reuse < 0) {
				pr_err("Async memdump: detect_pid_reuse failed "
				       "(task %d)\n", uid(item));
				close_image(pi);
				close_image(pmi);
				return -1;
	...
	if (max_iovs > 0) {
		iovs_fd = memfd_create("criu-fork-dump-iovs", MFD_CLOEXEC);
		if (iovs_fd < 0) {
			pr_perror("Async memdump: memfd_create(iovs) failed");
			goto err_pi;
		}
		iovs_bytes = (size_t)max_iovs * sizeof(struct iovec);
		mmap_bytes = iovs_bytes + aio_total_bytes;
		if (ftruncate(iovs_fd, mmap_bytes) < 0) {
			pr_perror("Async memdump: ftruncate(iovs_fd, %zu)",
				  mmap_bytes);
			goto err_pi;
		}
		iovs_map = mmap(NULL, mmap_bytes, PROT_READ | PROT_WRITE,
				MAP_SHARED, iovs_fd, 0);
		if (iovs_map == MAP_FAILED) {
			pr_perror("Async memdump: mmap(iovs_fd) failed");
			goto err_pi;
		}
		iovs = (struct iovec *)iovs_map;
		aio_blob = (char *)iovs_map + iovs_bytes;
	}

	if (aio_total_bytes > 0) {
		size_t off = 0;
		...
		list_for_each_entry(vma, &vma_area_list->h, list) {
			...
			len = vma->e->end - vma->e->start;
			local.iov_base = aio_blob + off;
			local.iov_len = len;
			remote.iov_base = decode_pointer(vma->e->start);
			remote.iov_len = len;

			n = process_vm_readv(item->pid->real, &local, 1,
					     &remote, 1, 0);
			if (n != (ssize_t)len) {
				pr_perror("Async memdump: process_vm_readv "
					  "aio @%#lx (%zd/%zu)",
					  (unsigned long)vma->e->start, n,
					  len);
				goto err_pi;
			}
			off += len;
	...
	if (max_iovs > 0) {
		size_t pmc_size = max((size_t)vma_area_list->nr_priv_pages_longest,
				      (size_t)1);
		if (pmc_init(&pmc, item->pid->real, &vma_area_list->h,
			     pmc_size * PAGE_SIZE) < 0) {
			pr_err("Async memdump: pmc_init failed (task %d)\n",
			       item->pid->real);
			goto err_pi;
		}
		pmc_inited = true;
	...
	list_for_each_entry(vma, &vma_area_list->h, list) {
		...
		if (vma_entry_is(vma->e, VMA_AREA_AIORING)) {
			size_t len = vma->e->end - vma->e->start;
			PagemapEntry pe = PAGEMAP_ENTRY__INIT;

			iovs[idx].iov_base = (void *)(uintptr_t)
				(HELPER_IOV_AIO_FLAG | (iovs_bytes + aio_off));
			iovs[idx].iov_len = len;
			idx++;
			aio_off += len;
			nr_pe_present += len / PAGE_SIZE;
		...
		dump_all = should_dump_entire_vma(vma->e);

		if (pmc_get_map(&pmc, vma) < 0) {
			pr_err("Async memdump: pmc_get_map failed for "
			       "vma %#lx-%#lx (task %d)\n",
			       (unsigned long)vma->e->start,
			       (unsigned long)vma->e->end, item->pid->real);
			goto err_pi;
		}

		for (vaddr = vma->e->start; vaddr < vma->e->end; ) {
			struct page_info pi_info = {};
			bool dumpable;
			unsigned int next_flags;

			if (should_dump_page(&pmc, vma->e, vaddr, &pi_info) < 0)
				goto err_pi;

			dumpable = dump_all || (pi_info.next == vaddr);
			...
			if (!dumpable) {
				...
				if (run_pages > 0) {
					...
					if (run_flags & PE_PRESENT) {
						iovs[idx].iov_base = decode_pointer(run_start);
						iovs[idx].iov_len = run_pages * PAGE_SIZE;
						idx++;
						nr_pe_present += run_pages;
					} else {
						nr_pe_parent += run_pages;
					}
					pe.vaddr = run_start;
					pe.nr_pages = run_pages;
					pe.has_nr_pages = true;
					pe.has_flags = true;
					pe.flags = run_flags;
					if (pb_write_one(pmi, &pe, PB_PAGEMAP) < 0)
				...
				vaddr = pi_info.next > vaddr ? pi_info.next
							     : vaddr + PAGE_SIZE;
				continue;
			}

			next_flags = (has_parent && page_in_parent(pi_info.softdirty))
					? PE_PARENT : PE_PRESENT;

			if (run_pages > 0 && next_flags != run_flags) {
				PagemapEntry pe = PAGEMAP_ENTRY__INIT;
	...
	args->nr_iovs = idx;
	args->iovs_bytes = iovs_bytes;
	args->aio_bytes = aio_total_bytes;

	if (iovs_map != MAP_FAILED) {
		munmap(iovs_map, mmap_bytes);
		iovs_map = MAP_FAILED;
		iovs = NULL;
		aio_blob = NULL;
	}
	...
	pr_info("Async memdump: %u iov runs (worst-case capacity %lu); "
		"PE_PRESENT %lu pages, PE_PARENT %lu pages%s "
		"(aio: %u rings, %zu bytes)\n",
		idx, max_iovs, nr_pe_present, nr_pe_parent,
		has_parent ? " (incremental)" : "",
		nr_aios, aio_total_bytes);

	close_image(pmi);
	pmi = NULL;

	image_fd = img_raw_fd(pi);
	if (image_fd < 0) {
		pr_err("Async memdump: img_raw_fd(pages-%u.img) failed\n",
		       pages_id);
		goto err_pi;
	}

	ret = compel_rpc_call(PARASITE_CMD_FORK_DUMP_PAGES, ctl);
	if (ret) {
		pr_err("Async memdump fork: rpc_call failed (pid: %d)\n",
		       item->pid->real);
		goto err_pi;
	}
	ret = compel_util_send_fd(ctl, image_fd);
	if (ret) {
		int sync_ret = compel_rpc_sync(PARASITE_CMD_FORK_DUMP_PAGES, ctl);
		pr_err("Async memdump fork: send_fd(image_fd) failed; "
		       "sync_ret=%d\n", sync_ret);
		goto err_pi;
	}
	ret = compel_util_send_fd(ctl, iovs_fd >= 0 ? iovs_fd : image_fd);
	if (ret) {
		int sync_ret = compel_rpc_sync(PARASITE_CMD_FORK_DUMP_PAGES, ctl);
		pr_err("Async memdump fork: send_fd(iovs_fd) failed; "
		       "sync_ret=%d\n", sync_ret);
		goto err_pi;
	}
	ret = compel_util_recv_fd(ctl, &helper_pidfd);
	if (ret) {
		int sync_ret = compel_rpc_sync(PARASITE_CMD_FORK_DUMP_PAGES, ctl);
		pr_err("Async memdump fork: recv_fd(helper_pidfd) failed; "
		       "sync_ret=%d\n", sync_ret);
		goto err_pi;
	}
	ret = compel_rpc_sync(PARASITE_CMD_FORK_DUMP_PAGES, ctl);
	if (ret) {
		pr_err("Async memdump fork failed (pid: %d)\n", item->pid->real);
		close(helper_pidfd);
		goto err_pi;
	}
	...
	pr_info("Async memdump helper forked (target real pid %d, helper "
		"vpid %d, %u iov runs to dump)\n",
		item->pid->real, args->child_pid, idx);

	if (opts.tfork.dumpd_sock < 0) {
		pr_err("dumpd_sock not initialized\n");
		close(helper_pidfd);
		return -1;
	}

	if (send_fd(opts.tfork.dumpd_sock, NULL, 0, helper_pidfd) < 0) {
		pr_perror("send_fd(helper_pidfd → dumpd) failed");
		close(helper_pidfd);
		return -1;
	}
	close(helper_pidfd);

	if (read(opts.tfork.dumpd_sock, &ack, 1) != 1) {
		pr_perror("read ack from dumpd failed");
		return -1;
	}
	if (ack != 1) {
		pr_err("dumpd reported reparent failure (ack=%d)\n", ack);
		return -1;
	}
	pr_debug("Helper vpid=%d reparented by dumpd\n", args->child_pid);
...
int prepare_mm_pid(struct pstree_item *i)
{
	...
	img = open_image(CR_FD_MM, O_RSTR, uid(i));
	if (!img)
		return -1;
	...
	if (ri->mm->n_vmas == 0) {
		 ...
		 */
		img = open_image(CR_FD_VMAS, O_RSTR, uid(i));
		if (!img)
			return -1;
	}
...
static int premap_private_vma(struct pstree_item *t, struct vma_area *vma, void **tgt_addr)
{
	...
	if (!vma_inherited(vma)) {
		 ...
		 * having any data in.
		 *
		 * MAP_NORESERVE: forced PROT_WRITE on huge PROT_NONE address-
		 * space reservations (chromium / V8 use tens of GB) flips them
		 * into committable mappings and ENOMEMs on overcommit-strict
		 * hosts. Pages still allocate lazily on write.
		 */
...
static int premap_priv_vmas(struct pstree_item *t, struct vm_area_list *vmas, void **at, struct page_read *pr)
{
	...
	list_for_each_entry(vma, &vmas->h, list) {
		...
		if (task_size_check(localpid(t), vma->e)) {
			ret = -1;
			break;
		}
...
int prepare_mappings(struct pstree_item *t)
{
	...
	ret = open_page_read(uid(t), &pr, PR_TASK);
	if (ret <= 0)
		return -1;
