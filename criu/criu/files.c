...
int find_unused_fd_pid(int pidns_id, pid_t pid_in_ns)
{
	struct pstree_item *task;

	task = pstree_item_by_pid_ns(pidns_id, pid_in_ns);
	if (!task) {
		pr_err("Invalid pid %d in namespace %d\n", pid_in_ns, pidns_id);
		return -1;
	}
	return find_unused_fd(task, -1);
}

int set_fds_event(struct pstree_item *item)
{
	bool is_set = !!test_and_set_bit_le(FDS_EVENT_BIT, &item->task_st_le_bits);
	if (!is_set)
...
static int fill_fd_params(struct pid *owner_pid, int fd, int lfd, struct fd_opts *opts, struct fd_parms *p)
{
	int ret;
	struct statfs fsbuf;
	struct fdinfo_common fdinfo = { .mnt_id = -1, .owner = owner_pid->local};
...
static struct fdinfo_list_entry *alloc_fle(struct pstree_item *item, FdinfoEntry *fe)
{
	struct fdinfo_list_entry *fle;
...
static void __collect_desc_fle(struct fdinfo_list_entry *new_le, struct file_desc *fdesc)
{
	...
	list_for_each_entry_reverse(le, &fdesc->fd_info_head, desc_list)
		if (pid_rst_prio_eq(le->real_pid, new_le->real_pid))
			break;
...
struct fdinfo_list_entry *collect_fd_to(struct pstree_item *item, FdinfoEntry *e, struct rst_info *rst_info,
					struct file_desc *fdesc, bool fake, bool force_master)
{
	struct fdinfo_list_entry *new_le;

	new_le = alloc_fle(item, e);
	if (new_le) {
		new_le->fake = (!!fake);
...
int collect_fd(struct pstree_item *item, FdinfoEntry *e, struct rst_info *rst_info, bool fake)
{
	struct file_desc *fdesc;

	pr_info("Collect fdinfo pid=%d(%d) fd=%d id=%#x\n", realpid(item), localpid(item), e->fd, e->id);
	...
	if (!collect_fd_to(item, e, rst_info, fdesc, fake, false))
		return -1;
...
int dup_fle(struct pstree_item *task, struct fdinfo_list_entry *ple, int fd, unsigned flags)
{
	...
	return collect_fd(task, e, rsti(task), false);
}
...
int prepare_fd_pid(struct pstree_item *item)
{
	...
	if (rsti(item)->fdt && rsti(item)->fdt->pid != uid(item))
		return 0;
	...
	while (1) {
		...
		ret = collect_fd(item, e, rst_info, false);
		if (ret < 0) {
...
static void transport_name_gen(struct sockaddr_un *addr, int *len, int uid)
{
	addr->sun_family = AF_UNIX;
	snprintf(addr->sun_path, UNIX_PATH_MAX, "x/crtools-fd-%d-%s", uid, criu_run_id);
	*len = SUN_LEN(addr);
	*addr->sun_path = '\0';
}
...
static int recv_fd_from_peer(struct fdinfo_list_entry *fle)
{
	...
	do {
		...
		if (!task_fle(current, tmp)) {
			pr_err("Unexpected fle %p, pid=%d(%d)\n", tmp, realpid(current), localpid(current));
			return -1;
		}
		if (plant_fd(tmp, fd))
			return -1;
...
static bool is_tfork_reserved_fd(int fd)
{
	int j;

	if (opts.tfork.ncopy_ready_fd >= 0 && fd == opts.tfork.ncopy_ready_fd)
		return true;

	if (!opts.tfork.active)
		return false;
	if (fd == opts.tfork.vma_cherrypick_fd)
		return true;
	if (opts.tfork.snap_root_fd >= 0 && fd == opts.tfork.snap_root_fd)
		return true;
	for (j = 0; j < opts.tfork.pidfd_map_nr; j++) {
		if (opts.tfork.pidfd_map[j].pidfd == fd)
			return true;
		if (opts.tfork.pidfd_map[j].memfd == fd)
			return true;
		if (opts.tfork.pidfd_map[j].pagemap_fd == fd)
			return true;
	}
	return false;
}
...
int prepare_fds(struct pstree_item *me)
{
	...
	if (rsti(me)->fdt) {
		...
		if (fdt->pid != uid(me)) {
			pr_info("File descriptor table is shared with uid %d\n", fdt->pid);
...
int restore_fs(struct pstree_item *me)
{
	...
	if (ri->fs_restored) {
		pr_debug("restore_fs: already done early (pre-userns-swap), skipping\n");
		return 0;
	}

	if (ri->root_fd >= 0 && ri->cwd_fd >= 0) {
		dd_root = ri->root_fd;
		dd_cwd = ri->cwd_fd;
		ri->root_fd = -1;
		ri->cwd_fd = -1;
...
int prepare_fs_pid(struct pstree_item *item)
{
	...
	img = open_image(CR_FD_FS, O_RSTR, uid(item));
	if (!img)
		goto out;
...
int shared_fdt_prepare(struct pstree_item *item)
{
	...
	if (!rsti(parent)->fdt) {
		...
		if (opts.tfork.active) {
			fdt->tfork_cherrypick_cpos =
				rst_mem_align_cpos(RM_SHREMAP);
			b = rst_mem_alloc(sizeof(*b), RM_SHREMAP);
			if (!b)
				return -1;
			futex_init(b);
		} else {
			fdt->tfork_cherrypick_cpos = 0;
		}
	} else
		fdt = rsti(parent)->fdt;
...
{
	struct inherit_fd *inh, *prev;

	list_for_each_entry(inh, &opts.inherit_fds, inh_list) {
		bool deduped = false;

		list_for_each_entry(prev, &opts.inherit_fds, inh_list) {
			if (prev == inh)
				break;
			if (prev->inh_fd == inh->inh_fd && prev->inh_fd >= 0) {
				inh->inh_fd_id = prev->inh_fd_id;
				inh->inh_fd = -1;
				deduped = true;
				break;
			}
...
int inherit_fd_for_each(int (*cb)(int fd, const char *key, void *arg), void *arg)
{
	struct inherit_fd *inh;
	int ret;

	list_for_each_entry(inh, &opts.inherit_fds, inh_list) {
		ret = cb(inh->inh_fd, inh->inh_id, arg);
		if (ret != 0)
			return ret;
	}
	return 0;
...
int open_transport_socket(void)
{
	...
	transport_name_gen(&saddr, &slen, uid(current));
