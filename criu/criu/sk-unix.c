	...
	*fown = p->fown;
	...
	if (ue->peer) {
		...
		if (sk->wqlen != 0) {
			 ...
			 *
			 * Exception: --unix-skip-in-flight (auto-enabled for
			 * tfork in cr-tfork.c). Tfork is a live local fork —
			 * the source stays alive, so dropping a few pending
			 * bytes on the in-flight socket just means the
			 * cloned-side workload retries on its fresh socket.
			 * Hit on KDE webtop in the wild: xclip's pending
			 * X11-server request bytes lived in this exact state
			 * and tripped sk-unix.c:506 across repeat clones.
			 */
			if (opts.unix_skip_in_flight) {
				pr_warn("unix: skipping in-flight icon socket %#x with %u bytes wqlen "
					"(--unix-skip-in-flight); pending bytes lost on the new image\n",
					ue->ino, sk->wqlen);
...
static struct fdinfo_list_entry *get_fle_for_task(struct file_desc *tgt, struct pstree_item *owner, bool force_master)
{
	 ...
	 */
	return collect_fd_to(owner, e, rsti(owner), tgt, true, force_master);
}
...
int unix_note_scm_rights(int id_for, uint32_t *file_ids, int *fds, int n_ids)
{
	...
	for (i = 0; i < n_ids; i++) {
		...
		pr_info("scm: add file %#x -> %d(%d)\n", tgt->id, realpid(owner), localpid(owner));
...
static int keep_deleted(struct unix_sk_info *ui)
{
	...
	ui->fdstore_id = fdstore_add_at(fd, FDSTORE_CANONICAL_ID(FDSTORE_P_UNIX_SK, ui->ue->id));
