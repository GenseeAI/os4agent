...
static int collect_remap_dead_process(struct reg_file_info *rfi, RemapFilePathEntry *rfe)
{
	...
	helper = get_or_create_helper_item(NULL, rfe->remap_id);
	if (!helper)
		return -1;
...
nomntns:
	...
	if (ret) {
		...
		if (opts.tfork.active && errno == ENOENT) {
			remap->rpath[0] = 0;
			return 0;
		}
			     ...
			     bool *fallback)
{
	...
	while (linkat_hard(lfd, "", mntns_root, link_name, ost->st_uid, ost->st_gid, AT_EMPTY_PATH) < 0) {
		if (errno == EXDEV) {
			pr_info("link_remap %s hit EXDEV; falling back to ghost-file dump\n",
				path);
			if (fallback)
				*fallback = true;
			return -1;
...
int dead_pid_conflict(void)
{
	...
	for (i = 0; i < n_dead_pids; i++) {
		...
		item = pstree_item_by_pid_ns(dead_pids[i].ns_id, dead_pids[i].pid);
		if (!item)
			continue;
		...
		pr_err("Conflict with a dead task with the same PID as of this thread %d(%d).\n",
...
static int have_seen_dead_pid(pid_t pid, u32 ns_id)
{
	int i;

	for (i = 0; i < n_dead_pids; i++) {
		if (dead_pids[i].pid == pid && dead_pids[i].ns_id == ns_id)
			return 1;
	}

	if (xrealloc_safe(&dead_pids, sizeof(*dead_pids) * (n_dead_pids + 1)))
		return -1;
	dead_pids[n_dead_pids].pid = pid;
	dead_pids[n_dead_pids].ns_id = ns_id;
	n_dead_pids++;

	return 0;
}

static int dump_dead_process_remap(pid_t pid, u32 ns_id, u32 id)
{
	RemapFilePathEntry rpe = REMAP_FILE_PATH_ENTRY__INIT;
	int ret;

	ret = have_seen_dead_pid(pid, ns_id);
	if (ret < 0)
		return -1;
	if (ret) {
	...
	rpe.orig_id = id;
	rpe.remap_id = pid;
	rpe.has_remap_ns_id = true;
	rpe.remap_ns_id = ns_id;
	rpe.has_remap_type = true;
...
static int check_path_remap(struct fd_link *link, const struct fd_parms *parms, int lfd, u32 id, struct ns_id *nsid)
{
	...
	if (parms->fs_type == PROC_SUPER_MAGIC) {
		...
		if (pid != 0) {
			...
			if (is_dead) {
				...
				ns = proc_read_ns(pid, &pid_ns_desc);
				if (ns) {
					ns_id = ns->id;
				} else if (root_item && root_item->ids &&
					   root_item->ids->has_pid_ns_id) {
					ns_id = root_item->ids->pid_ns_id;
				} else {
					pr_err("Failed to read pid namespace for pid %d\n", pid);
					return -1;
				}
				return dump_dead_process_remap(pid, ns_id, id);
			}
		}
	...
	if (ret < 0) {
		...
		if (errno == ENOENT) {
			...
			if (opts.tfork.active && opts.tfork.memdump_async)
				return dump_ghost_remap(rpath + 1, ost, lfd, id, nsid);
...
static bool is_stateless_mem_dev(const struct fd_parms *p, const char *path)
{
	if (!S_ISCHR(p->stat.st_mode))
		return false;
	if (major(p->stat.st_rdev) != MEM_MAJOR)
		return false;
	switch (minor(p->stat.st_rdev)) {
	case MEM_MINOR_NULL:
	case MEM_MINOR_ZERO:
	case MEM_MINOR_FULL:
...
static int find_host_mountpoint_for_dev(dev_t dev, char *out, size_t outlen)
{
	FILE *f;
	char buf[4096];
	...
	while (fgets(buf, sizeof(buf), f) != NULL) {
		...
		char root[PATH_MAX], mp[PATH_MAX];
		if (sscanf(buf, "%d %d %u:%u %s %s",
			   &mid, &parent, &dev_maj, &dev_min, root, mp) < 6)
			continue;
		if (makedev(dev_maj, dev_min) == dev) {
			snprintf(out, outlen, "%s", mp);
			fclose(f);
			return 0;
		}
	}
...
int dump_one_reg_file(int lfd, u32 id, const struct fd_parms *p)
{
	...
	if (mi == NULL) {
		if (opts.shell_job && is_tty(p->stat.st_rdev, p->stat.st_dev)) {
		...
		} else if (is_stateless_mem_dev(p, link->name + 1)) {
			pr_info("Bypassing unresolvable mount=%d for fd=%d %s — "
				"stateless MEM_MAJOR char dev, will reopen "
				"canonical path from restored mntns\n",
				p->mnt_id, p->fd, link->name + 1);
			stateless_dev = true;
		} else if (opts.tfork.active) {

			struct mount_info *fb;
			bool path_rewritten = false;
			for (fb = mntinfo; fb != NULL; fb = fb->next) {
				if (fb->s_dev == kdev_to_odev(p->stat.st_dev) && mnt_is_dir(fb))
					break;
			}
			if (fb == NULL) {
				static const char DEL_SUFFIX[] = " (deleted)";
				const char *fpath = &link->name[1];
				size_t flen = strlen(fpath);
				bool deleted = (flen >= sizeof(DEL_SUFFIX) - 1 &&
						strcmp(fpath + flen - (sizeof(DEL_SUFFIX) - 1),
						       DEL_SUFFIX) == 0);
				if (deleted) {
					struct mount_info *best = NULL;
					size_t best_len = 0;
					size_t plen = flen - (sizeof(DEL_SUFFIX) - 1);
					struct mount_info *m;
					for (m = mntinfo; m != NULL; m = m->next) {
						...
						if (mlen > plen)
							continue;
						if (memcmp(fpath, mp, mlen) != 0)
							continue;

						if (mlen != plen && mp[mlen - 1] != '/' && fpath[mlen] != '/')
							continue;
						if (mlen > best_len ||
						    (mlen == best_len && best == NULL)) {
							best = m;
							best_len = mlen;
						}
					}
					if (best != NULL) {
						pr_info("tfork: mount=%d not in source mountinfo "
							"(sdev=%lx absent too), fell back to "
							"path-prefix match mnt_id=%d for "
							"deleted fd=%d %s\n",
							p->mnt_id, (unsigned long)p->stat.st_dev,
							best->mnt_id, p->fd, fpath);
						fb = best;
					}
				}
			}
			if (fb == NULL) {
				...
				size_t best_match_off = 0;
				const char *fpath = &link->name[1];
				struct mount_info *m;
				for (m = mntinfo; m != NULL; m = m->next) {
					const char *cursor, *found, *needle;
					size_t rl;
					if (!mnt_is_dir(m))
						continue;
					if (m->root == NULL ||
				...
				if (best != NULL) {
					...
					nsmp = best->ns_mountpoint ? best->ns_mountpoint : "/";
					if (nsmp[0] == '.')
						nsmp++;
					if (nsmp[0] == '\0')
						nsmp = "/";
					nl = strlen(nsmp);
					while (nl > 1 && nsmp[nl - 1] == '/')
						nl--;

					suffix = fpath + best_match_off;
					sl = strlen(suffix);

					if (sl > 0 && suffix[0] != '/') {
						pr_err("tfork: unexpected non-slash suffix "
						       "'%s' after root='%s' match in '%s'\n",
						       suffix, best->root, fpath);
						return -1;
					}

					if (nl == 1 && nsmp[0] == '/') {

						n = snprintf(new_name, sizeof(new_name),
							     ".%s",
							     sl > 0 ? suffix : "/");
					} else {
						n = snprintf(new_name, sizeof(new_name),
					...
					if (n < 0 || (size_t)n >= sizeof(new_name)) {
						pr_err("tfork: rewritten in-source path too long "
						       "(mnt_id=%d root=%s suffix=%s)\n",
						       best->mnt_id, best->root, suffix);
						return -1;
					}
					pr_info("tfork: mount=%d not in source mountinfo "
						"(sdev=%lx absent too, deleted-path also "
						"absent), fell back to mount-root-path "
						"match: mnt_id=%d root=%s — rewrote "
						"fd=%d %s -> %s\n",
						p->mnt_id, (unsigned long)p->stat.st_dev,
						best->mnt_id, best->root, p->fd,
						fpath, new_name + 1);
					memcpy(link->name, new_name, (size_t)n + 1);
					link->len = (size_t)n;
					fb = best;
					path_rewritten = true;
				}
			...
			if (fb != NULL) {
				if (!path_rewritten) {
					char host_mp[PATH_MAX];
					if (find_host_mountpoint_for_dev(p->stat.st_dev, host_mp, sizeof(host_mp)) == 0) {
						size_t hl = strlen(host_mp);
						if (link->len > hl + 1 &&
						    strncmp(&link->name[1], host_mp, hl) == 0 &&
						    link->name[hl + 1] == '/') {
							size_t copy = strlen(&link->name[1 + hl]) + 1;
							memmove(&link->name[1], &link->name[1 + hl], copy);
							link->len -= hl;
							pr_info("tfork: stripped host mp '%s', path now '%s'\n",
								host_mp, &link->name[1]);
						}
					}
					pr_info("tfork: mount=%d not in source mountinfo, "
						"fell back to mnt_id=%d (sdev=%lx match) for fd=%d %s\n",
						p->mnt_id, fb->mnt_id,
						(unsigned long)p->stat.st_dev,
						p->fd, link->name + 1);
				}
				mi = fb;
			} else {
				pr_err("Can't lookup mount=%d for fd=%d path=%s (tfork sdev=%lx fallback also failed)\n",
				       p->mnt_id, p->fd, link->name + 1,
				       (unsigned long)p->stat.st_dev);
				return -1;
			}
	...
	if (!skip_for_shell_job && !stateless_dev && mnt_is_overmounted(mi)) {
		pr_err("Open files on overmounted mounts are not supported yet; mount=%d fd=%d path=%s\n",
		       p->mnt_id, p->fd, link->name + 1);
		return -1;
	}

	if (!stateless_dev && p->mnt_id >= 0 && (root_ns_mask & CLONE_NEWNS)) {

		rfe.mnt_id = mi ? mi->mnt_id : p->mnt_id;
		rfe.has_mnt_id = true;
	}
	...
	if (!skip_for_shell_job && !stateless_dev && check_path_remap(link, p, lfd, id, mi->nsid))
		return -1;
...
int do_open_reg_noseek_flags_snap(int ns_root_fd, struct reg_file_info *rfi, void *arg)
{
	int fd;
	u32 flags = *(u32 *)arg;

	if (opts.tfork.snap_root_fd >= 0 && !rfi->remap) {
		fd = openat(opts.tfork.snap_root_fd, rfi->path,
			    flags & ~O_TMPFILE);
		if (fd >= 0) {

			if (opts.file_validation_method ==
			    FILE_VALIDATION_BUILD_ID && rfi->rfe->n_build_id) {
				struct stat st;
				if (fstat(fd, &st) == 0 &&
				    validate_with_build_id(fd, &st, rfi) == -1) {
					pr_info("snap_root_fd %s: build-ID mismatch, falling back to ns_root_fd\n",
						rfi->path);
					close(fd);
					goto fallback;
				}
		...
		if (errno != ENOENT) {
			pr_perror("Can't open file %s via snap_root_fd %d",
				  rfi->path, opts.tfork.snap_root_fd);
			return -1;
		}
	}

fallback:
	return do_open_reg_noseek_flags(ns_root_fd, rfi, arg);
}
...
static int open_filemap(int pid, struct vma_area *vma)
{
	...
	if (ctx.flags != flags || ctx.desc != vma->vmfd) {
		if (plugin_fd >= 0) {
			...
			ret = open_path(vma->vmfd,
					do_open_reg_noseek_flags_snap,
					&flags);
		}
		if (ret < 0)
			return ret;
...
static int tfork_open_remapped_reg(struct file_desc *d, struct reg_file_info *rfi)
{
	struct fdinfo_list_entry *fle_m;
	struct ghost_file *gf;
	int j, pidfd = -1, fd;

	if (!opts.tfork.active || !rfi->remap || rfi->remap->is_dir)
		return -ENOENT;

	gf = container_of(rfi->remap, struct ghost_file, remap);
	if (tfork_inode_has_watch(gf->dev, gf->ino)) {
		pr_info("tfork: %#x:%lx is watched, skipping pidfd_getfd for reg file %#x\n",
			gf->dev, (unsigned long)gf->ino, rfi->rfe->id);
		return -ENOENT;
	}
	...
	if (fle_m->fake) {
		pr_info("tfork: file_master for reg file %#x is a fake fle (SCM_RIGHTS in-flight); skipping pidfd_getfd\n",
			rfi->rfe->id);
		return -ENOENT;
	}

	for (j = 0; j < opts.tfork.pidfd_map_nr; j++) {
		if (opts.tfork.pidfd_map[j].uid == uid(fle_m->task)) {
			pidfd = opts.tfork.pidfd_map[j].pidfd;
			break;
		}
	}
	if (pidfd < 0)
	...
	fd = syscall(SYS_pidfd_getfd, pidfd, fle_m->fe->fd, 0);
	if (fd < 0) {
		pr_perror("tfork: pidfd_getfd(%d, %d) failed for reg file %#x",
			  pidfd, fle_m->fe->fd, rfi->rfe->id);
		return -1;
	}

	pr_info("tfork: opened reg file %#x (task uid %d fd %d) via pidfd_getfd -> fd %d\n",
		rfi->rfe->id, uid(fle_m->task), fle_m->fe->fd, fd);
	return fd;
}
...
static int open_fe_fd(struct file_desc *fd, int *new_fd)
{
	...
	tmp = tfork_open_remapped_reg(fd, container_of(fd, struct reg_file_info, d));
	if (tmp != -ENOENT) {
		if (tmp < 0)
			return -1;
		*new_fd = tmp;
