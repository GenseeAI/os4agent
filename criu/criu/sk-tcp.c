...
static void tcp_unlock_one(struct inet_sk_desc *sk)
{
	...
	if (sk->rfd >= 0) {
		...
		socklen_t wlen = sizeof(wopt);
		if (getsockopt(sk->rfd, SOL_TCP, TCP_REPAIR_WINDOW,
			       &wopt, &wlen) == 0 && wopt.rcv_wnd == 0) {
			wopt.rcv_wnd = 1;
			if (wopt.max_window < 1)
				wopt.max_window = 1;
			if (setsockopt(sk->rfd, SOL_TCP, TCP_REPAIR_WINDOW,
				       &wopt, sizeof(wopt)) == 0)
				pr_debug("nudged rcv_wnd 0->1 on rfd=%d\n",
					 sk->rfd);
		}
	}
