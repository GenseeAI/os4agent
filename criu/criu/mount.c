...
int prepare_mnt_ns(void)
{
	...
	for (nsid = ns_ids; nsid != NULL; nsid = nsid->next) {
		...
		nsid->mnt.nsfd_id = fdstore_add_at(fd, FDSTORE_CANONICAL_ID(FDSTORE_P_MNTNS_NSFD, nsid->id));
		close(fd);
		...
		if (fd < 0)
			goto err;
		nsid->mnt.root_fd_id = fdstore_add_at(fd, FDSTORE_CANONICAL_ID(FDSTORE_P_MNTNS_ROOT, nsid->id));
