...
const char *sfd_type_name(enum sfd_type type)
{
	static const char *names[] = {
		...
		[CAPBYPASS_FD_OFF] = __stringify_1(CAPBYPASS_FD_OFF),
		[PKEY_STATE_FD_OFF] = __stringify_1(PKEY_STATE_FD_OFF),
...
static void sfds_protection_bug(enum sfd_type type)
{
	...
	print_stack_trace(current ? realpid(current) : 0);
	BUG();
}
...
int install_service_fd(enum sfd_type type, int fd)
{
	...
	BUG_ON((int)type <= SERVICE_FD_MIN || (int)type >= SERVICE_FD_MAX);
	if (sfds_protected && !test_bit(type, sfd_map) && type != PROC_SELF_FD_OFF)
...
{
	clear_bit(PROC_SELF_FD_OFF, sfd_map);
}
