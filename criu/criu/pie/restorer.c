...
#include "kernel_module/vma_cherrypick/uapi_vma_cherrypick.h"
...
static int lsm_set_label(char *label, char *type, int procfd, pid_t outer_tid)
{
	int ret = -1, len, lsmfd;
	...
	std_sprintf(path, "self/task/%ld/attr/%s", (long)outer_tid, type);

	lsmfd = sys_openat(procfd, path, O_WRONLY, 0);
	if (lsmfd < 0) {
		pr_err("LSM: lsm_set_label failed openat %s path=%s procfd=%d outer_tid=%d ret=%d (errno %d)\n",
		       type, path, procfd, outer_tid, lsmfd, -lsmfd);
		if (task_entries_local) {
			atomic_cmpxchg(&task_entries_local->cr_err, 0, -lsmfd);
			futex_abort_and_wake_err(&task_entries_local->nr_in_progress, -lsmfd);
		}
		return -1;
	}
...
static int restore_creds(struct thread_creds_args *args, int procfd, int lsm_type, uid_t uid, pid_t outer_tid)
{
	...
	if (args->groups) {
		...
		if (n != ce->n_groups || memcmp(gids, args->groups, n * sizeof(*gids))) {
			...
			if (ret == -EPERM || ret == -EINVAL) {
				...
				pr_warn("setgroups([%zu gids]) -> %d; user_ns disallows it, skipping (best effort)\n",
					ce->n_groups, ret);
			} else if (ret) {
...
skip_xids:
	...
	if (lsm_type != LSMTYPE__SELINUX) {
		 ...
		 * threads are created.
		 */
		if (lsm_set_label(args->lsm_profile, "current", procfd, outer_tid) < 0)
			return -1;
	}

	/* Also set the sockcreate label for all threads */
	if (lsm_set_label(args->lsm_sockcreate, "sockcreate", procfd, outer_tid) < 0)
		return -1;
...
__visible long __export_restore_thread(struct thread_restore_args *args)
{
	...
	ret = restore_creds(args->creds_args, args->ta->proc_fd, args->ta->lsm_type, args->ta->uid,
			    args->tid_in_ns[args->ns_level - 1]);
...
__visible long __export_restore_task(struct task_restore_args *args)
{
	...
	*args->breakpoint = rst_sigreturn;
	...
	if (args->tfork_mode) {
		...
		unsigned long bs_start = (unsigned long)bootstrap_start;
		unsigned long bs_end = bs_start + bootstrap_len;

		if (bs_start > 0) {
			ret = sys_munmap(NULL, bs_start);
			if (ret) {
				pr_err("tfork: unmap 0-%lx: %ld\n",
				       bs_start, ret);
				goto core_restore_end;
			}
		}
		if (bs_end < args->task_size) {
			ret = sys_munmap((void *)bs_end,
					 args->task_size - bs_end);
			if (ret) {
				pr_err("tfork: unmap %lx-%lx: %ld\n",
				       bs_end, args->task_size, ret);
				goto core_restore_end;
			}
		}

		args->vdso_maps_rt.vdso_start = VDSO_BAD_ADDR;
		args->vdso_maps_rt.vvar_start = VVAR_BAD_ADDR;
	} else {
		if (unmap_old_vmas((void *)args->premmapped_addr, args->premmapped_len, bootstrap_start,
				   bootstrap_len, args->task_size))
			goto core_restore_end;
	...
	if (args->tfork_mode) {
		struct vma_cherrypick_args cp_args;

		pr_info("tfork: restorer entering vma_cherrypick path\n");

		for (i = 0; i < args->vmas_n; i++) {
			bool is_wipeon_dontfork, is_full_memcopy_anon;
			vma_entry = args->vmas + i;

			if (!vma_entry_is(vma_entry, VMA_AREA_REGULAR) &&
			    !vma_entry_is(vma_entry, VMA_AREA_AIORING) &&
			    !vma_entry_is(vma_entry, VMA_AREA_VDSO) &&
			...
			if (vma_entry_is(vma_entry, VMA_AREA_SYSVIPC)) {
				...
				if (va != vma_entry->start) {
					pr_err("tfork: sysv shmat %lx -> %lx\n",
					       (unsigned long)vma_entry->start, va);
					goto core_restore_end;
				}
				continue;
			}

			if ((vma_entry->status & VMA_AREA_MEMFD) &&
			    !(vma_entry->status & VMA_FILE_PRIVATE)) {
				va = restore_mapping(vma_entry);
				if (va != vma_entry->start) {
					pr_err("tfork: memfd mmap %lx -> %lx\n",
					       (unsigned long)vma_entry->start, va);
					goto core_restore_end;
				}
				continue;
			...
			if (vma_entry->flags & MAP_DROPPABLE) {
				va = sys_mmap((void *)vma_entry->start,
					      ...
					      -1, 0);
				if (va != vma_entry->start) {
					pr_err("tfork: mmap for droppable VMA %lx failed\n",
			...
			is_wipeon_dontfork = vma_entry->has_madv &&
				(vma_entry->madv & ((1ul << MADV_WIPEONFORK) |
						    (1ul << MADV_DONTFORK)));
			is_full_memcopy_anon = args->tfork_full_memcopy &&
				!(vma_entry->status &
				  (VMA_FILE_SHARED | VMA_FILE_PRIVATE |
				   VMA_AREA_MEMFD)) &&
				!vma_entry_is(vma_entry, VMA_AREA_VDSO) &&
				!vma_entry_is(vma_entry, VMA_AREA_VVAR) &&
				!vma_entry_is(vma_entry, VMA_AREA_AIORING);

			if (is_wipeon_dontfork || is_full_memcopy_anon) {
				unsigned long len = vma_entry_len(vma_entry);
				bool filter_resident;

				va = sys_mmap((void *)vma_entry->start, len,
					      vma_entry->prot | PROT_WRITE,
					      (vma_entry->flags & ~MAP_SHARED) |
					      MAP_FIXED | MAP_PRIVATE |
					      MAP_ANONYMOUS,
					      -1, 0);
				if (va != vma_entry->start) {
					pr_err("tfork: mmap fallback VMA %lx failed: %lx\n",
					       (unsigned long)vma_entry->start, va);
					goto core_restore_end;
				}

				filter_resident = is_full_memcopy_anon &&
					args->target_pagemap_fd >= 0;

				if (filter_resident) {
					const uint64_t RESIDENT_MASK =
						(1ULL << 63) | (1ULL << 62);
					uint64_t pmap_buf[512];
					unsigned long total_pages = len >> PAGE_SHIFT;
					unsigned long pmap_off =
						(vma_entry->start >> PAGE_SHIFT) *
						sizeof(uint64_t);
					unsigned long pi = 0;
					...
					while (pi < total_pages) {
						...
						r = sys_pread(args->target_pagemap_fd,
							      (char *)pmap_buf,
						...
						if (r != (long)(batch *
								sizeof(uint64_t))) {
							pr_err("tfork: pagemap pread VMA %lx pi=%lx: %ld (want %lu)\n",
							       (unsigned long)vma_entry->start,
							       pi, r,
							       batch * sizeof(uint64_t));
							goto core_restore_end;
						...
						while (bi < batch) {
							...
							while (copied < run_len) {
								long rr = sys_pread(
									args->target_memfd,
									(char *)vma_entry->start + va_off + copied,
									run_len - copied,
									vma_entry->start + va_off + copied);
								if (rr <= 0) {
									pr_err("tfork: pread fallback %lx off=%lx: %ld\n",
									       (unsigned long)vma_entry->start,
									       va_off + copied,
									       rr);
									goto core_restore_end;
				...
				if (!(vma_entry->prot & PROT_WRITE)) {
					ret = sys_mprotect((void *)vma_entry->start,
							   len, vma_entry->prot);
					if (ret) {
						pr_err("tfork: mprotect fallback %lx: %ld\n",
						       (unsigned long)vma_entry->start, ret);
						goto core_restore_end;
					}
				}
			...
			cp_args.target_id = args->target_pidfd;
			cp_args.flags = VMA_CHERRYPICK_FLAG_PIDFD;
			cp_args.src_addr = vma_entry->start;
			cp_args.dst_addr = vma_entry->start;
			cp_args.file_fd = -1;
			cp_args._pad = 0;

			if ((vma_entry->status &
			     (VMA_FILE_SHARED | VMA_FILE_PRIVATE)) &&
			    (int64_t)vma_entry->fd >= 0) {
				cp_args.flags |= VMA_CHERRYPICK_FLAG_FILE_FD;
				if (vma_entry->status & VMA_FILE_PRIVATE)
					cp_args.flags |=
						VMA_CHERRYPICK_FLAG_FILE_FD_COW;
				cp_args.file_fd = (int)vma_entry->fd;
			}

			ret = sys_ioctl(args->vma_cherrypick_fd, VMA_CHERRYPICK,
					(unsigned long)&cp_args);
			if (ret) {
				pr_err("vma_cherrypick failed for %lx: %ld\n",
				       (unsigned long)vma_entry->start, ret);
				goto core_restore_end;
			}

			if ((cp_args.flags & VMA_CHERRYPICK_FLAG_FILE_FD) &&
			    (vma_entry->status & VMA_CLOSE))
				sys_close(cp_args.file_fd);
		}

		for (i = 0; i < args->vmas_n; i++) {
			vma_entry = args->vmas + i;

			if (!(vma_entry->flags & MAP_LOCKED))
				continue;
			if (!vma_entry_is(vma_entry, VMA_AREA_REGULAR))
				continue;

			ret = sys_mlock((void *)vma_entry->start,
					vma_entry_len(vma_entry));
			if (ret)
				pr_warn("tfork: mlock(%lx, %lu) failed: %ld\n",
					(unsigned long)vma_entry->start,
					(unsigned long)vma_entry_len(vma_entry),
					ret);
		}

		if (args->tfork_fdt_done) {
			futex_inc_and_wake(args->tfork_fdt_done);
			futex_wait_while_lt(args->tfork_fdt_done,
					    args->tfork_fdt_nr);
		}

		hfd_top = args->vma_cherrypick_fd;
		if (args->tfork_snap_root_fd > hfd_top)
			hfd_top = args->tfork_snap_root_fd;
		for (hfd = args->tfork_pidfd_base; hfd <= hfd_top; hfd++)
			sys_close(hfd);

		if (args->tfork_ncopy_ready_fd >= 0)
			sys_close(args->tfork_ncopy_ready_fd);

		goto tfork_skip_page_restore;
	}
...
tfork_skip_page_restore:
	if (args->tfork_mode) {

		for (i = 0; i < args->vmas_n; i++) {
			if (vma_entry_is(&args->vmas[i], VMA_AREA_VDSO))
				args->vdso_maps_rt.vdso_start = args->vmas[i].start;
			if (vma_entry_is(&args->vmas[i], VMA_AREA_VVAR))
				args->vdso_maps_rt.vvar_start = args->vmas[i].start;
		}
		has_vdso_proxy = false;
		vdso_update_gtod_addr(&args->vdso_maps_rt);
	} else if (vdso_proxify(&args->vdso_maps_rt, &has_vdso_proxy,
				args->vmas, args->vmas_n,
	...
	/*
	 * Finally restore madvise() bits.
	 * tfork: vma_cherrypick clears VM_LOCKED_MASK and does not
	 * preserve all madvise flags, so we must re-apply them.
	 */
	for (i = 0; i < args->vmas_n; i++) {
		unsigned long m;
		...
		for (m = 0; m < sizeof(vma_entry->madv) * 8; m++) {
			if (vma_entry->madv & (1ul << m)) {
				...
				if (ret && args->tfork_mode)
					pr_warn("madvise(%" PRIx64 ", %" PRIu64 ", %ld) "
						"failed with %ld (non-fatal in tfork)\n",
	...
	if (args->lsm_type == LSMTYPE__SELINUX) {
		if (lsm_set_label(args->t->creds_args->lsm_profile, "current",
				  args->proc_fd,
				  args->t->tid_in_ns[args->t->ns_level - 1]) < 0)
			goto core_restore_end;
	}
	...
	ret = restore_creds(args->t->creds_args, args->proc_fd, args->lsm_type, args->uid,
			    args->t->tid_in_ns[args->t->ns_level - 1]);
