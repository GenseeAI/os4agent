...
helper_main(struct parasite_fork_dump_pages_args *args)
{
	int image_fd = args->image_fd;
	unsigned int nr_iovs = args->nr_iovs;
	char *map_base = (char *)args->iovs_map_base;

	if (image_fd >= 0 && map_base && nr_iovs > 0) {
		struct iovec *iovs = (struct iovec *)map_base;
		unsigned int i;

		for (i = 0; i < nr_iovs; i++) {
			unsigned long base = (unsigned long)iovs[i].iov_base;
			const void *src;

			if (base & HELPER_IOV_AIO_FLAG)
				src = map_base +
				      (base & ~HELPER_IOV_AIO_FLAG);
			else
				src = (const void *)base;

			(void)sys_write(image_fd, src, iovs[i].iov_len);
		}
	}
...
static int fork_dump_pages(struct parasite_fork_dump_pages_args *args)
{
	long pid;
	int tsock;
	int image_fd;
	...
	register long r10_arg __asm__("r10") = 0;
	register long r8_arg  __asm__("r8")  = 0;

	tsock = parasite_get_rpc_sock();
	image_fd = recv_fd(tsock);
	if (image_fd < 0) {
		pr_err("fork_dump_pages: recv_fd(image_fd) failed: %d\n",
		       image_fd);
		return -1;
	}
	iovs_fd = recv_fd(tsock);
	if (iovs_fd < 0) {
		pr_err("fork_dump_pages: recv_fd(iovs_fd) failed: %d\n",
		       iovs_fd);
		sys_close(image_fd);
		return -1;
	}
	args->image_fd = image_fd;
	args->iovs_fd = iovs_fd;
	args->iovs_map_base = 0;

	if (args->nr_iovs > 0) {
		unsigned long bytes = args->iovs_bytes + args->aio_bytes;
		long mret;

		mret = sys_mmap(NULL, bytes, PROT_READ, MAP_SHARED, iovs_fd, 0);
		if (mret < 0 && mret > -4096) {
			pr_err("fork_dump_pages: sys_mmap(iovs_fd=%d, %lu) "
			       "failed: %ld\n", iovs_fd, bytes, mret);
			sys_close(iovs_fd);
			sys_close(image_fd);
			return -1;
		}
		args->iovs_map_base = (unsigned long)mret;
	}

	stack = sys_mmap(NULL, 0x8000, PROT_READ | PROT_WRITE,
			 MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	if (stack < 0 && stack > -4096) {
		pr_err("fork_dump_pages: helper stack mmap failed: %ld\n",
		       stack);
		if (args->iovs_map_base)
			sys_munmap((void *)args->iovs_map_base,
				   args->iovs_bytes + args->aio_bytes);
		sys_close(iovs_fd);
		sys_close(image_fd);
		return -1;
	}
	args->helper_stack_top = (unsigned long)stack + 0x8000;
	...
	if (pid < 0) {
		pr_err("fork_dump_pages: sys_clone failed: %ld\n", pid);
		return -1;
	}

	args->child_pid = (int)pid;

	helper_pidfd = sys_pidfd_open(pid, 0);
	if (helper_pidfd < 0) {
		pr_err("fork_dump_pages: pidfd_open(helper vpid=%ld) "
		       "failed: %d\n", pid, helper_pidfd);
		return -1;
	}
	ret = send_fd(tsock, NULL, 0, helper_pidfd);
	sys_close(helper_pidfd);
	if (ret < 0) {
		pr_err("fork_dump_pages: send_fd(helper_pidfd) failed: %d\n",
		       ret);
		return -1;
	}
	...
	if (args->iovs_map_base)
		sys_munmap((void *)args->iovs_map_base,
			   args->iovs_bytes + args->aio_bytes);
	if (args->helper_stack_top)
		sys_munmap((void *)(args->helper_stack_top - 0x8000),
			   0x8000);
...
int parasite_daemon_cmd(int cmd, void *args)
{
	...
	case PARASITE_CMD_DUMP_CGROUP:
		...
		break;
	case PARASITE_CMD_FORK_DUMP_PAGES:
		ret = fork_dump_pages(args);
		break;
	default:
