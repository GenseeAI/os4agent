...
int criu_local_set_tfork_snap_root(criu_opts *opts, const char *path)
{
	opts->rpc->tfork_snap_root = strdup(path);
	if (!opts->rpc->tfork_snap_root)
		return -ENOMEM;
	return 0;
}
...
{
	return criu_local_set_tfork_snap_root(global_opts, path);
}

int criu_local_add_tfork_snap_root(criu_opts *opts, const char *path)
{
	int nr;
	char **a, *e = NULL;
	...
	nr = opts->rpc->n_tfork_snap_roots + 1;
	a = realloc(opts->rpc->tfork_snap_roots, nr * sizeof(*a));
	if (!a)
		goto err;

	a[nr - 1] = e;
	opts->rpc->tfork_snap_roots = a;
	opts->rpc->n_tfork_snap_roots = nr;
	return 0;
err:
	if (e)
		free(e);
...
{
	return criu_local_add_tfork_snap_root(global_opts, path);
}

int criu_local_add_tfork_snap_mount(criu_opts *opts, const char *path)
{
	int nr;
	char **a, *e = NULL;
	...
	nr = opts->rpc->n_tfork_snap_mount + 1;
	a = realloc(opts->rpc->tfork_snap_mount, nr * sizeof(*a));
	if (!a)
		goto err;

	a[nr - 1] = e;
	opts->rpc->tfork_snap_mount = a;
	opts->rpc->n_tfork_snap_mount = nr;
	return 0;
err:
	if (e)
		free(e);
...
{
	return criu_local_add_tfork_snap_mount(global_opts, path);
}

void criu_local_set_tfork_memdump(criu_opts *opts, bool val)
{
	opts->rpc->has_tfork_memdump = true;
	opts->rpc->tfork_memdump = val;
}

void criu_set_tfork_memdump(bool val)
{
	criu_local_set_tfork_memdump(global_opts, val);
}

void criu_local_set_tfork_memdump_async(criu_opts *opts, bool val)
{
	opts->rpc->has_tfork_memdump_async = true;
	opts->rpc->tfork_memdump_async = val;
}

void criu_set_tfork_memdump_async(bool val)
{
	criu_local_set_tfork_memdump_async(global_opts, val);
}

void criu_local_set_tfork_copies(criu_opts *opts, int copies)
{
	opts->rpc->has_tfork_copies = true;
	opts->rpc->tfork_copies = copies;
}

void criu_set_tfork_copies(int copies)
{
	criu_local_set_tfork_copies(global_opts, copies);
}

void criu_local_set_tfork_dumpd_parent_pid(criu_opts *opts, int pid)
{
	opts->rpc->has_tfork_dumpd_parent_pid = true;
	opts->rpc->tfork_dumpd_parent_pid = pid;
}

void criu_set_tfork_dumpd_parent_pid(int pid)
{
	criu_local_set_tfork_dumpd_parent_pid(global_opts, pid);
}

void criu_local_set_tfork_full_memcopy(criu_opts *opts, bool val)
{
	opts->rpc->has_tfork_full_memcopy = true;
	opts->rpc->tfork_full_memcopy = val;
}

void criu_set_tfork_full_memcopy(bool val)
{
	criu_local_set_tfork_full_memcopy(global_opts, val);
}

int criu_local_add_tfork_copy_args(criu_opts *opts, int copy_idx, const char *args)
{
	CriuTforkCopyArgs *e = NULL, **a;
	int nr;
	...
	if (!e)
		goto err;
	criu_tfork_copy_args__init(e);
	e->copy_idx = copy_idx;
	e->args = strdup(args);
	if (!e->args)
		goto err;

	nr = opts->rpc->n_tfork_copy_args + 1;
	a = realloc(opts->rpc->tfork_copy_args, nr * sizeof(*a));
	if (!a)
		goto err;

	a[nr - 1] = e;
	opts->rpc->tfork_copy_args = a;
	opts->rpc->n_tfork_copy_args = nr;
	return 0;
err:
	if (e) {
		if (e->args)
...
{
	return criu_local_add_tfork_copy_args(global_opts, copy_idx, args);
}
...
int criu_local_tfork(criu_opts *opts)
{
	int ret = -1;
	CriuReq req = CRIU_REQ__INIT;
	CriuResp *resp = NULL;
...
int criu_tfork(void)
{
	return criu_local_tfork(global_opts);
}
