...
static inline void futex_abort_and_wake_err(futex_t *f, int err)
{
	if (err > 0)
		futex_set_and_wake(f, (uint32_t)(int)-err);
	else
...
#define __NR_futex_waitv 449
#else
#define __NR_futex_waitv -1
#endif
#endif

static inline long sys_futex_waitv(struct futex_waitv *waiters, unsigned int nr_futexes,
				  unsigned int flags, struct timespec *timeout, clockid_t clockid)
{
#if __NR_futex_waitv > 0
	long ret = syscall(__NR_futex_waitv, waiters, nr_futexes, flags, timeout, clockid);
	if (ret == -1 && errno != 0)
		return -errno;
	return ret;
#else
