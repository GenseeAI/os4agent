...
static int child_main(int sync_fd, int host_utsns_fd)
{
	...
	log("child in nested user_ns, waiting for CLONE-only grant");
	...
	if (rc < 0) {
		log("FAIL: clone3 under CLONE-only grant returned %s", strerror(errno));
		return 2;
	}
	if (rc == 0)
		_exit(0);
	waitpid(rc, NULL, 0);
	log("OK: clone3 passed (CLONE class granted)");

	ret = setns(host_utsns_fd, CLONE_NEWUTS);
	if (ret == 0) {
		log("FAIL: setns(uts) succeeded under CLONE-only grant");
		return 3;
	}
	if (errno != EPERM) {
		log("FAIL: setns(uts) returned %s, expected EPERM", strerror(errno));
		return 4;
	}
	log("OK: setns(uts) stayed EPERM (SETNS class NOT granted)");
...
int main(void)
{
	...
	host_utsns_fd = open("/proc/self/ns/uts", O_RDONLY);
	if (host_utsns_fd < 0) {
		perror("open /proc/self/ns/uts");
		return 1;
	}
	...
	if (child == 0) {
		close(sync_pair[0]);
		close(dev_fd);
		_exit(child_main(sync_pair[1], host_utsns_fd));
	}
	close(sync_pair[1]);
	...
	cba.cap_mask     = 0;
	cba.syscall_mask = CRIU_CB_SC_CLONE;
	...
	log("test_narrow_syscall_mask: OK");
	return 0;
}
