"""Agent-S default prompts — vendored from
``simular-ai/Agent-S/gui_agents/s3/memory/procedural_memory.py``
(and patched copy at ``vikranth22446/Agent-S @ add-env-backends``).
Apache-2.0 licensed by simular-ai.

Kept here so a workload can inspect and override prompts without
following the submodule. To re-sync after upstream changes:

    cp agents/agent-s/Agent-S/gui_agents/s3/memory/procedural_memory.py \\
       agents/agent-s/workloads/prompts/agent_s_default.py
...
AGENT_S_DEFAULT_PROMPTS: dict[str, str] = {
    "REFLECTION_ON_TRAJECTORY": PROCEDURAL_MEMORY.REFLECTION_ON_TRAJECTORY,
    "PHRASE_TO_WORD_COORDS_PROMPT": PROCEDURAL_MEMORY.PHRASE_TO_WORD_COORDS_PROMPT,
    "CODE_AGENT_PROMPT": PROCEDURAL_MEMORY.CODE_AGENT_PROMPT,
    "CODE_SUMMARY_AGENT_PROMPT": PROCEDURAL_MEMORY.CODE_SUMMARY_AGENT_PROMPT,
    "BEHAVIOR_NARRATOR_SYSTEM_PROMPT": PROCEDURAL_MEMORY.BEHAVIOR_NARRATOR_SYSTEM_PROMPT,
    "FORMATTING_FEEDBACK_PROMPT": PROCEDURAL_MEMORY.FORMATTING_FEEDBACK_PROMPT,
