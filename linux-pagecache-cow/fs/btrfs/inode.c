...
cache_acl:
	...
	case S_IFREG:
		...
		if (READ_ONCE(sysctl_filecow_test_large_folios))
			mapping_set_large_folios(vfs_inode->i_mapping);
#endif
		break;
	case S_IFDIR:
...
again:
	...
	if (!folio_test_uptodate(folio)) {
		...
		folio_lock(folio);
		if (!folio_belongs_to(folio, mapping)) {
			folio_unlock(folio);
			folio_put(folio);
			goto again;
		}
...
again:
	...
	folio = __filemap_get_folio(mapping, index,
				    FGP_LOCK | FGP_ACCESSED | FGP_CREAT | FGP_WRITE,
				    mask);
	if (IS_ERR(folio)) {
		if (only_release_metadata)
...
#ifdef CONFIG_FILECOW
static void btrfs_iget_maybe_fork(struct btrfs_inode *snap)
{
	struct btrfs_root *snap_root = snap->root;
	struct btrfs_fs_info *fs_info;
	...
	if (!snap_root)
		return;
	fs_info = snap_root->fs_info;

	if (!S_ISREG(snap->vfs_inode.i_mode))
		return;

	if (READ_ONCE(snap->vfs_inode.i_data.ro))
		return;

	if (btrfs_root_readonly(snap_root))
		return;

	if (btrfs_is_empty_uuid(snap_root->root_item.parent_uuid))
		return;

	if (snap->vfs_inode.i_data.ro)
		return;

	ret = btrfs_uuid_tree_resolve_subvol(fs_info,
					     snap_root->root_item.parent_uuid,
					     BTRFS_UUID_KEY_SUBVOL, &src_subvol);
	if (ret)
		return;

	src_root = btrfs_get_fs_root(fs_info, src_subvol, true);
	if (IS_ERR(src_root))
		return;

	args.ino = btrfs_ino(snap);
	while (true) {
		struct btrfs_root *next_root;
		u64 next_subvol;

		args.root = src_root;
		hashval = btrfs_inode_hash(args.ino, src_root);
		src_inode = ilookup5(fs_info->sb, hashval,
				     btrfs_find_actor, &args);
		if (src_inode)
			break;

		if (btrfs_is_empty_uuid(src_root->root_item.parent_uuid))
			break;

		ret = btrfs_uuid_tree_resolve_subvol(fs_info,
				src_root->root_item.parent_uuid,
				BTRFS_UUID_KEY_SUBVOL, &next_subvol);
		if (ret)
			break;

		next_root = btrfs_get_fs_root(fs_info, next_subvol, true);
		if (IS_ERR(next_root))
			break;

		btrfs_put_root(src_root);
		src_root = next_root;
		extra_hops++;
	}

	if (extra_hops)
		atomic_long_add(extra_hops, &filecow_stat_iget_chain_hops);
	...
	if (!btrfs_is_empty_uuid(src_root->root_item.received_uuid)) {
		iput(src_inode);
		btrfs_put_root(src_root);
		return;
	}
	...
	if (S_ISREG(src_inode->i_mode) && src_inode != &snap->vfs_inode) {

		struct filecow_layer *ro_now =
			READ_ONCE(snap->vfs_inode.i_data.ro);
		if (ro_now)
			pr_warn_ratelimited(
				"filecow: btrfs_iget_maybe_fork pre-fork ro flipped "
				"snap=%px ino=%llu root=%llu ro_now=%px src_inode=%px\n",
				snap, btrfs_ino(snap),
				snap_root ? snap_root->root_key.objectid : 0,
				ro_now, src_inode);
		ret = address_space_fork(&snap->vfs_inode.i_data,
					 src_inode->i_mapping);
		if (!ret && extra_hops)
			atomic_long_inc(&filecow_stat_iget_chain_admits);
	}

	iput(src_inode);
}
#else
static inline void btrfs_iget_maybe_fork(struct btrfs_inode *snap) {}
#endif
				 ...
				 size_t length)
{
	...
	inode = folio_to_inode(folio);
	fs_info = inode->root->fs_info;
	tree = &inode->io_tree;
	inode_evicting = inode_state_read_once(&inode->vfs_inode) & I_FREEING;
...
#ifdef CONFIG_FILECOW
	.folio_extent_shared = btrfs_folio_extent_shared,
	.folio_extents_shared_bulk = btrfs_folios_extent_shared_bulk,
#endif
};
