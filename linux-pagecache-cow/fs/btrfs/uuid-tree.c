...
int btrfs_uuid_tree_resolve_subvol(struct btrfs_fs_info *fs_info,
				   const u8 *uuid, u8 type, u64 *subid_out)
{
	...
	slot = path->slots[0];
	item_size = btrfs_item_size(eb, slot);
	if (!IS_ALIGNED(item_size, sizeof(u64)) || item_size < sizeof(u64))
		return -EUCLEAN;

	offset = btrfs_item_ptr_offset(eb, slot);
	read_extent_buffer(eb, &subid_le, offset, sizeof(subid_le));
	*subid_out = le64_to_cpu(subid_le);
	return 0;
}
