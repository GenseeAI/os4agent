...
again:
	...
	if (!folio_belongs_to(folio, inode->vfs_inode.i_mapping) ||
	    (page_start >= size)) {
	...
	if (folio_test_filecow(folio)) {
		...
		gfp_t gfp = mapping_gfp_mask(inode->vfs_inode.i_mapping);
		pgoff_t cow_index = folio->index;
		new_folio = filecow_cow_folio(inode->vfs_inode.i_mapping,
					      folio, folio->index, gfp);
		if (IS_ERR(new_folio)) {
		...
		unmap_mapping_pages(inode->vfs_inode.i_mapping, cow_index,
				    1, false);
		ret = 0;
		goto out;
	}
