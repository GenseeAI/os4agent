		...
		int error)
{
	...
	if (error)
		fserror_report_io(folio_mapping(folio)->host, FSERR_BUFFERED_READ,
...
{
	trace_iomap_release_folio(folio_mapping(folio)->host, folio_pos(folio),
			folio_size(folio));
...
{
	trace_iomap_invalidate_folio(folio_mapping(folio)->host,
		...
		const struct iomap_write_ops *write_ops, size_t len)
{
	...
	if (iter->iomap.flags & IOMAP_F_FOLIO_BATCH) {
		 ...
		 */
		folio_lock(folio);
		if (unlikely(!folio_belongs_to(folio, iter->inode->i_mapping))) {
