 ...
 * belongs to.  For layer-tagged folios (folio-share-design-v10 §16.12), this
 * is the layer's primary inode — a weakly-referenced sharer host that may be
 * NULL transiently (between sharer evictions) per §29.4 follow-up.  Returns
 * NULL in that case; callers walking layer folios on snapshot filesystems
 * should be prepared.  On non-snapshot filesystems the layer-tagged path is
 * dead by §20.28 and this helper preserves its original non-NULL contract.
 *
...
static inline struct inode *folio_inode(struct folio *folio)
{
	...
	return mapping ? mapping->host : NULL;
}
