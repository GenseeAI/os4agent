...
#ifdef CONFIG_FILECOW
FOLIO_FLAG(filecow_origin, FOLIO_HEAD_PAGE)
	__FOLIO_SET_FLAG(filecow_origin, FOLIO_HEAD_PAGE)
	__FOLIO_CLEAR_FLAG(filecow_origin, FOLIO_HEAD_PAGE)
#else
FOLIO_FLAG_FALSE(filecow_origin)
	__FOLIO_SET_FLAG_NOOP(filecow_origin)
	__FOLIO_CLEAR_FLAG_NOOP(filecow_origin)
#endif
...
static __always_inline bool folio_test_filecow(const struct folio *folio)
{
	return ((unsigned long)folio->mapping & FOLIO_MAPPING_FILECOW) != 0;
}

static __always_inline struct filecow_layer *folio_to_filecow_layer(const struct folio *folio)
{
	unsigned long m = (unsigned long)folio->mapping;
	return (struct filecow_layer *)(m & ~FOLIO_MAPPING_FILECOW);
}
