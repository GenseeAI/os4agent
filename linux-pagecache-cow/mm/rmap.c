		...
		const struct page *page, const struct vm_area_struct *vma)
{
	if (folio_test_anon(folio)) {
		...
		return -EFAULT;
	} else if (!folio_belongs_to((struct folio *)folio,
...
#define FILECOW_RMAP_MAX_SHARERS	64

static void rmap_walk_file_filecow(struct folio *folio,
				   struct rmap_walk_control *rwc, bool locked)
{
	struct filecow_layer *layer = folio_to_filecow_layer(folio);
	struct address_space *snapshot[FILECOW_RMAP_MAX_SHARERS];
	struct address_space *m;
	int n = 0, i;

	spin_lock(&layer->sharers_lock);
	list_for_each_entry(m, &layer->sharers, filecow_link) {
		if (n >= FILECOW_RMAP_MAX_SHARERS) {
			WARN_ONCE(1, "filecow: sharers list exceeds %d; tail peers skipped on rmap walk\n",
				  FILECOW_RMAP_MAX_SHARERS);
			break;
		}
		if (igrab(m->host))
			snapshot[n++] = m;
	...
	for (i = 0; i < n; i++) {
		bool stop = false;
		__rmap_walk_file(folio, snapshot[i], folio->index,
				 folio_nr_pages(folio), rwc, locked);
		if (rwc->done && rwc->done(folio))
			stop = true;
		iput(snapshot[i]->host);
		if (stop) {
			i++;
			break;
		}
		...
		struct rmap_walk_control *rwc, bool locked)
{
	...
	if (folio_test_filecow(folio)) {
		rmap_walk_file_filecow(folio, rwc, locked);
		return;
	}
#endif
