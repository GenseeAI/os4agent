...
static vm_fault_t fault_dirty_shared_page(struct vm_fault *vmf)
{
	 ...
	 *
	 * Use folio_mapping() rather than folio_raw_mapping(): for a
	 * FILECOW-tagged folio, the raw value is (layer | 0x4) which is
	 * not a struct address_space *.  folio_mapping() unwraps to the
	 * layer's primary inode mapping (folio-share-design-v10 §20.8).
	 * In practice the CoW path detaches the folio from the layer
	 * before this site sees it, so this is defensive — but cheaper
	 * than auditing every future caller.
	 */
 ...
 * to unmap it finally.
 *
 * Layer-tagged (filecow) folios are mapped via VMAs in **every** sharer's
 * address space, not just one — folio->mapping is a tagged layer pointer
 * (not an address_space *).  Walk @layer->sharers and tear down PTEs in
 * each sharer's i_mmap rbtree (folio-share-design-v10 §11.5 / §20.8).
 */
void unmap_mapping_folio(struct folio *folio)
{
	...
	if (folio_test_filecow(folio)) {
		...
		struct address_space *sharer;
		struct inode **snap;
		int n = 0, cap = 0, i;

		spin_lock(&layer->sharers_lock);
		list_for_each_entry(sharer, &layer->sharers, filecow_link)
			cap++;
		...
		snap = kmalloc_array(cap, sizeof(*snap), GFP_NOFS);
		if (!snap) {
			WARN_ON_ONCE(1);
			return;
		}
		...
		list_for_each_entry(sharer, &layer->sharers, filecow_link) {
			if (n >= cap)
				break;
			if (sharer->host && igrab(sharer->host))
				snap[n++] = sharer->host;
		}
		spin_unlock(&layer->sharers_lock);

		for (i = 0; i < n; i++) {
			struct address_space *sm = snap[i]->i_mapping;

			i_mmap_lock_read(sm);
			if (unlikely(!RB_EMPTY_ROOT(&sm->i_mmap.rb_root)))
				unmap_mapping_range_tree(&sm->i_mmap,
						first_index, last_index,
						&details);
			i_mmap_unlock_read(sm);
			filecow_defer_iput(snap[i]);
		}
