		...
		unsigned long nr_to_read, unsigned long lookahead_size)
{
	...
	atomic_long_inc(&filecow_stat_ra_unbounded_calls);
	if (mapping_filecow_layer(mapping))
		atomic_long_inc(&filecow_stat_ra_unbounded_layer_calls);
	...
	while (i < nr_to_read) {
		...
		if (!folio && mapping_filecow_layer(mapping)) {
			struct folio *layer_folio = filecow_lookup(mapping, index + i, gfp_mask);
			if (layer_folio) {
				folio_put(layer_folio);
				atomic_long_inc(&filecow_stat_ra_chain_hit);
				read_pages(ractl);
				ractl->_index += min_nrpages;
				i = ractl->_index + ractl->_nr_pages - index;
				continue;
			}
			atomic_long_inc(&filecow_stat_ra_chain_miss);
		}
		...
		struct file_ra_state *ra)
{
	...
	atomic_long_inc(&filecow_stat_ra_order_calls);
	...
	if (mapping_filecow_layer(mapping)) {
		atomic_long_inc(&filecow_stat_ra_order_layer_fallback);
		ra->order = 0;
		goto fallback;
	}
