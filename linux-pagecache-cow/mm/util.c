...
struct address_space *folio_mapping(const struct folio *folio)
{
	...
	m = (unsigned long)mapping;

#ifdef CONFIG_FILECOW
	if (unlikely(m & FOLIO_MAPPING_FILECOW)) {
		struct filecow_layer *layer = folio_to_filecow_layer(folio);
		struct inode *primary;

		primary = READ_ONCE(layer->primary_inode);
		if (!primary)
			return NULL;
		return primary->i_mapping;
	}
#endif

	if (m & FOLIO_MAPPING_FLAGS)
		return NULL;
