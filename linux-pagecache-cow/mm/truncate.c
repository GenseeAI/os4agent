...
{
	const struct address_space_operations *aops = folio_mapping(folio)->a_ops;
...
int truncate_inode_folio(struct address_space *mapping, struct folio *folio)
{
	...
	if (folio_test_filecow(folio)) {
		filecow_drop_local_slot(mapping, folio);
		return 0;
	}
				    ...
				    unsigned long min_order)
{
	 ...
	 */
	if (ret && !shmem_mapping(folio_mapping(folio))) {
 ...
 */
bool truncate_inode_partial_folio(struct address_space *mapping,
				  struct folio *folio, loff_t start, loff_t end)
{
	 ...
	 */
	if (!mapping_inaccessible(folio_mapping(folio)))
	...
	min_order = mapping_min_folio_order(folio_mapping(folio));
	...
	if (!try_folio_split_or_unmap(folio, split_at, min_order)) {
		...
		if (folio_test_large(folio2) &&
		    folio_mapping(folio2) == folio_mapping(folio))
...
no_split:
	...
	if (folio_test_dirty(folio))
		return false;
	truncate_inode_folio(folio_mapping(folio), folio);
	return true;
}
				...
				loff_t lstart, uoff_t lend)
{
	...
	if (!IS_ERR(folio)) {
		...
		if (!truncate_inode_partial_folio(mapping, folio, lstart, lend)) {
	...
	while (index < end) {
		...
		for (i = 0; i < folio_batch_count(&fbatch); i++) {
			...
			if (folio_test_filecow(folio)) {
				filecow_drop_local_slot_no_folio_lock(mapping,
								      folio);
				continue;
			}
...
static int folio_launder(struct address_space *mapping, struct folio *folio)
{
	if (!folio_test_dirty(folio))
		return 0;
	if (!folio_belongs_to(folio, mapping) || mapping->a_ops->launder_folio == NULL)
		return 0;
			   ...
			   gfp_t gfp)
{
	...
	if (folio_test_filecow(folio)) {
		long nr = __filecow_drop_local_slot_locked(mapping, folio, true);
		xa_unlock_irq(&mapping->i_pages);
		if (mapping_shrinkable(mapping))
			inode_lru_list_add(mapping->host);
		spin_unlock(&mapping->host->i_lock);
	...
	__filemap_remove_folio(folio,
		folio_test_filecow_origin(folio) ? xa_make_tombstone() : NULL);
	xa_unlock_irq(&mapping->i_pages);
	if (mapping_shrinkable(mapping))
		inode_lru_list_add(mapping->host);
				  ...
				  pgoff_t start, pgoff_t end)
{
	...
	if (mapping_filecow_layer(mapping) && end != ULONG_MAX) {
		pgoff_t i;

		spin_lock(&mapping->host->i_lock);
		xa_lock_irq(&mapping->i_pages);
		for (i = start; i <= end; i++) {
			void *prev = __xa_store(&mapping->i_pages, i,
						xa_make_tombstone(),
						GFP_ATOMIC);
			if (prev && !xa_is_value(prev)) {
				struct folio *displaced = prev;
				long nr = folio_nr_pages(displaced);

				mapping->nrpages -= nr;
				folio_put_refs(displaced, nr);
			}
			if (i == ULONG_MAX)
				break;
		}
...
void truncate_pagecache_range(struct inode *inode, loff_t lstart, loff_t lend)
{
	...
	if (mapping_filecow_layer(mapping) && lend != LLONG_MAX) {
		pgoff_t first = lstart >> PAGE_SHIFT;
		pgoff_t last = lend >> PAGE_SHIFT;
		pgoff_t i;
