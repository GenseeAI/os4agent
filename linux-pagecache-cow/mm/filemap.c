				   ...
				   struct folio *folio, void *shadow)
{
	...
	if (WARN_ON_ONCE(folio_test_filecow(folio))) {
		pr_warn("filecow: page_cache_delete on tagged folio "
			"folio=%px ->mapping=%px caller_mapping=%px idx=%lu\n",
			folio, (void *)folio->mapping, mapping,
			folio->index);
		dump_stack();
	}
		...
		struct folio *folio)
{
	...
	if (test_bit(AS_KERNEL_FILE, &folio_mapping(folio)->flags))
...
void filemap_remove_folio(struct folio *folio)
{
	...
	if (folio_test_filecow_origin(folio))
		shadow = xa_make_tombstone();
			     ...
			     struct folio_batch *fbatch)
{
	...
	xas_for_each(&xas, folio, ULONG_MAX) {
		...
		if (folio_test_filecow_origin(folio))
			xas_store(&xas, xa_make_tombstone());
		else
			xas_store(&xas, NULL);
...
repeat:
	...
	if (xa_is_value(folio)) {
		folio = NULL;
	} else if (!folio && mapping_filecow_layer(mapping)) {
		folio = filecow_lookup(mapping, index, gfp);
	}
	if (!folio)
		goto no_page;
	...
	if (fgp_flags & FGP_LOCK) {
		...
		if (unlikely(!folio_belongs_to(folio, mapping))) {
			folio_unlock(folio);
			folio_put(folio);
			goto repeat;
		}
	...
	if ((fgp_flags & FGP_WRITE) && folio_test_filecow(folio)) {
		struct folio *new_folio;
		bool was_locked = !!(fgp_flags & FGP_LOCK);
		if (!was_locked)
			folio_lock(folio);
		new_folio = filecow_cow_folio(mapping, folio, index, gfp);
		if (IS_ERR(new_folio)) {
			folio_unlock(folio);
			folio_put(folio);
			if (PTR_ERR(new_folio) == -EAGAIN)
				goto repeat;
			return new_folio;
		}
		folio_unlock(folio);
...
retry:
	...
	if (!folio_batch_count(fbatch) && mapping_filecow_layer(mapping)) {
		...
		for (i = index; i < last_index; i++) {
			...
			rcu_read_lock();
			entry = xa_load(&mapping->i_pages, i);
			rcu_read_unlock();
			if (xa_is_value(entry))
				continue;

			f = filecow_lookup(mapping, i, gfp);
			if (!f)
				continue;
			any_faulted = true;
			folio_put(f);
		...
		if (any_faulted)
			filemap_get_read_batch(mapping, index, last_index - 1,
					       fbatch);
	}
	if (!folio_batch_count(fbatch)) {
...
vm_fault_t filemap_page_mkwrite(struct vm_fault *vmf)
{
	...
	if (folio_test_filecow(folio)) {
		...
		new_folio = filecow_cow_folio(mapping, folio, folio->index, gfp);
		if (IS_ERR(new_folio)) {
			folio_unlock(folio);
			ret = (PTR_ERR(new_folio) == -ENOMEM) ?
				VM_FAULT_OOM : VM_FAULT_NOPAGE;
			goto out;
		}
		folio_unlock(folio);
		folio_unlock(new_folio);
		folio_put(new_folio);
		unmap_mapping_pages(mapping, folio->index, 1, false);
		ret = VM_FAULT_NOPAGE;
		goto out;
	}
			     ...
			     loff_t start, loff_t end)
{
	...
	if (mapping_filecow_layer(mapping) && nr != ULONG_MAX && first <= last) {
		pgoff_t i;
		...
		for (i = first; i <= last; i++) {
			__xa_cmpxchg(&mapping->i_pages, i, NULL,
				     xa_make_tombstone(), GFP_ATOMIC);
			if (i == ULONG_MAX)
				break;
		}
		xa_unlock_irq(&mapping->i_pages);
