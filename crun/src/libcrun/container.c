...
static int
tfork_save_per_copy_status (libcrun_context_t *context,
                            libcrun_checkpoint_restore_t *cr_options,
                            const char *bundle_cwd, libcrun_error_t *err)
{
  int copies = cr_options->tfork_copies;
  int i;
  ...
  for (i = 1; i < copies; i++)
    {
      ...
      cleanup_free char *config_buf = NULL;
      cleanup_free char *config_name = NULL;
      cleanup_free char *pidfile_path = NULL;
      cleanup_free char *pidfile_buf = NULL;
      cleanup_free char *pidfile_name = NULL;
      cleanup_free char *children_path = NULL;
      cleanup_free char *children_buf = NULL;
      cleanup_free char *clone_id = NULL;
      cleanup_free char *owner = NULL;
      cleanup_free char *state_dir = NULL;
      cleanup_free char *dest_config = NULL;
      char rootfs_buf[64];
      char created[35];
      char err_buffer[1024];
      const char *ann_path[] = { "annotations", "io.podman.tfork.clone-id", NULL };
      yajl_val tree;
      yajl_val val;
      pid_t restore_pid, init_pid;
      libcrun_container_status_t status = { 0 };
      char *space;
      size_t config_len = 0;
      int ret;

      ret = xasprintf (&config_name, "config-%d.json", i);
      if (UNLIKELY (ret < 0))
        return crun_make_error (err, errno, "asprintf config-%d.json", i);

      ret = append_paths (&config_path, err, bundle_cwd, config_name, NULL);
      if (UNLIKELY (ret < 0))
        return ret;

      ret = read_all_file (config_path, &config_buf, &config_len, err);
      if (UNLIKELY (ret < 0))
        return crun_error_wrap (err, "tfork: read per-copy config `%s`", config_path);

      tree = yajl_tree_parse (config_buf, err_buffer, sizeof (err_buffer));
      if (UNLIKELY (tree == NULL))
        return crun_make_error (err, 0, "tfork: parse `%s`: %s", config_path, err_buffer);

      val = yajl_tree_get (tree, ann_path, yajl_t_string);
      if (val == NULL)
        {
          yajl_tree_free (tree);
          return crun_make_error (err, 0,
                                  "tfork: `%s` missing annotation `io.podman.tfork.clone-id`",
                                  config_path);
        }
      clone_id = xstrdup (YAJL_GET_STRING (val));
      yajl_tree_free (tree);
      if (clone_id == NULL)
        return crun_make_error (err, ENOMEM, "strdup clone-id");

      ret = xasprintf (&pidfile_name, "tfork.pid.copy%d", i);
      if (UNLIKELY (ret < 0))
        return crun_make_error (err, errno, "asprintf tfork.pid.copy%d", i);
      ...
      if (UNLIKELY (ret < 0))
        return crun_error_wrap (err, "tfork: read per-copy pidfile `%s`", pidfile_path);

      restore_pid = (pid_t) strtoll (pidfile_buf, NULL, 10);
      if (UNLIKELY (restore_pid <= 0))
        return crun_make_error (err, 0, "tfork: invalid pid in `%s`", pidfile_path);

      init_pid = restore_pid;
      ret = xasprintf (&children_path, "/proc/%d/task/%d/children", restore_pid, restore_pid);
      if (LIKELY (ret >= 0))
        {
          libcrun_error_t tmp_err = NULL;
          int rret = read_all_file (children_path, &children_buf, NULL, &tmp_err);
          if (LIKELY (rret >= 0) && children_buf != NULL)
            {
              ...
              if (children_buf[0] != '\0')
                {
                  pid_t child = (pid_t) strtoll (children_buf, NULL, 10);
                  if (child > 0)
                    init_pid = child;
                }
            }
      ...
      ret = libcrun_status_check_directories (context->state_root, clone_id, err);
      if (UNLIKELY (ret < 0))
        return ret;

      ret = libcrun_get_state_directory (&state_dir, context->state_root, clone_id, err);
      if (UNLIKELY (ret < 0))
        return ret;
      ret = append_paths (&dest_config, err, state_dir, "config.json", NULL);
      if (UNLIKELY (ret < 0))
        return ret;
      ret = write_file (dest_config, config_buf, config_len, err);
      if (UNLIKELY (ret < 0))
        return crun_error_wrap (err, "tfork: write per-copy `%s`", dest_config);

      owner = get_user_name (geteuid ());
      get_current_timestamp (created, sizeof (created));
      snprintf (rootfs_buf, sizeof (rootfs_buf), "rootfs-%d", i);

      status.pid = init_pid;
      status.bundle = (char *) bundle_cwd;
      status.rootfs = rootfs_buf;
      status.created = created;
      status.owner = owner;
      status.detached = 1;
      status.systemd_cgroup = context->systemd_cgroup;
      status.external_descriptors = (char *) "[]";

      ret = libcrun_write_container_status (context->state_root, clone_id, &status, err);
      if (UNLIKELY (ret < 0))
        return ret;
    }
...
int
libcrun_container_tfork (libcrun_context_t *context, libcrun_container_t *container,
                         libcrun_checkpoint_restore_t *cr_options, libcrun_error_t *err)
{
  ...
  if (UNLIKELY (context == NULL))
    return crun_make_error (err, EINVAL, "internal error: context is NULL");

  if (UNLIKELY (cr_options->source_state == NULL))
    return crun_make_error (err, EINVAL, "--source-state is required for `crun tfork`");
  ...
  ret = libcrun_container_tfork_linux (container, cr_options, &clone_pid, err);
  if (UNLIKELY (ret < 0))
    return ret;
  ...
  if (context->pid_file)
    {
      char buf[32];
      int buf_len = snprintf (buf, sizeof (buf), "%d", clone_pid);
      ret = write_file_at_with_flags (AT_FDCWD, O_CREAT | O_TRUNC, 0700,
                                      context->pid_file, buf, buf_len, err);
      if (UNLIKELY (ret < 0))
        return ret;
    }
  ...
  cwd = getcwd (NULL, 0);
  if (UNLIKELY (cwd == NULL))
    return crun_make_error (err, errno, "getcwd failed");

  owner = get_user_name (geteuid ());
  get_current_timestamp (created, sizeof (created));

  status.pid = clone_pid;
  status.bundle = cwd;
  status.rootfs = container->container_def->root && container->container_def->root->path
                      ? container->container_def->root->path
                      : "";
  status.created = created;
  status.owner = owner;
  status.detached = 1;
  ...
  ret = libcrun_write_container_status (context->state_root, context->id, &status, err);
  if (UNLIKELY (ret < 0))
    return ret;

  return tfork_save_per_copy_status (context, cr_options, cwd, err);
}
